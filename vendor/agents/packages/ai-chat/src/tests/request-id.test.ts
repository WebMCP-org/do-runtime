...
describe("requestId in OnChatMessageOptions", () => {
  it("replays a duplicate completed request id without running inference twice", async () => {
    const room = crypto.randomUUID();
    const { ws: original } = await connectChatWS(
      `/agents/test-chat-agent/${room}`
    );
    const agentStub = await getAgentByName(env.TestChatAgent, room);
    const requestId = "duplicate-completed-request";
    ...
    await sendChatAndWaitForDone(original, requestId, messages);
    expect(await agentStub.getChatMessageCallCountForTest()).toBe(1);

    const { ws: replacement } = await connectChatWS(
      `/agents/test-chat-agent/${room}`
    );
    const replayOffer = new Promise<Record<string, unknown> | null>(
      (resolve) => {
        const timeout = setTimeout(() => resolve(null), 2000);
        replacement.addEventListener("message", (event: MessageEvent) => {
          const frame = JSON.parse(event.data as string) as Record<
            string,
            unknown
          >;
          if (
            frame.type === MessageType.CF_AGENT_STREAM_RESUMING &&
            frame.id === requestId
          ) {
            clearTimeout(timeout);
            resolve(frame);
    ...
    expect(await replayOffer).toEqual(
  ...
  it("deduplicates a no-stream request after its terminal response is lost", async () => {
    const room = crypto.randomUUID();
    const { ws: original } = await connectChatWS(
      `/agents/test-chat-agent/${room}`
    );
    const agentStub = await getAgentByName(env.TestChatAgent, room);
    const requestId = "duplicate-no-stream-request";
    const messages: ChatMessage[] = [
      {
        id: "no-stream-user",
        role: "user",
        parts: [{ type: "text", text: "Do not create a stream" }]
      }
    ];
    const persistedReceipt = new Promise<void>((resolve) => {
      original.addEventListener("message", (event: MessageEvent) => {
        const frame = JSON.parse(event.data as string) as Record<
          string,
          unknown
        >;
        if (
          frame.type === MessageType.CF_AGENT_STREAM_PENDING &&
          frame.id === requestId &&
    ...
    original.send(
      JSON.stringify({
        ...
        init: {
          method: "POST",
          body: JSON.stringify({ messages, noResponse: true, delayMs: 100 })
        }
      })
    );
    await persistedReceipt;
    original.close(1000);
