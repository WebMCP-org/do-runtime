...
> extends Agent<Env, State, Props> {
    ...
    const handleMessage = async (
      ...
      if (typeof message === "string") {
        ...
        if (event.type === "chat-request" && event.init.method === "POST") {
          ...
          // Vendor divergence (2026-08-14 ledger entry): replayed requests
          // retain their id. Route duplicates to the pre-stream/resumable
          // owner before admitting another inference turn — the client's
          // transport outbox replays a byte-identical frame across route
          // changes until it sees a receipt, and upstream would run inference
          // a second time. `claimRequest` also takes the pre-stream claim that
          // upstream takes with `_preStream.begin()` below.
          if (
            !(await this._resumeHandshake().claimRequest(
              connection,
              chatMessageId
            ))
          ) {
          ...
          try {
            ...
            // Vendor divergence: the claim above is taken before this await,
            // so a throw here must settle it (upstream's `begin()` runs after
            // the span and cannot strand a claim).
            this._settlePreStreamTurn(chatMessageId);
            throw error;
          }

          // Outer finally so the accepted pre-stream turn ALWAYS settles
          // (#1784). The resume handshake claims the id before its terminal
          // lookup await, ahead of the pre-turn-body steps below
          ...
          try {
            ...
            try {
              ...
              // Vendor divergence (2026-08-14 ledger entry): the request has
              // now crossed the persistence boundary, so its client can stop
              // replaying it (`STREAM_PENDING { persisted: true }`) and a turn
              // that settles without a stream still leaves a receipt.
              this._preStream.markPersisted(chatMessageId, connection);
            } finally {
   ...
   *
   * Vendor divergence: this is not `async` (upstream declares it `async` and
   * both producers `await` it before broadcasting). On `@mcp-b/do-runtime`
   * every transformed `await` resumes in a fresh actor slice, so awaiting an
   * already-settled promise still admits another event: a `STREAM_RESUME_ACK`
   * delivered in that gap replays this chunk and leaves
   * `_pendingResumeConnections`, and the broadcast that follows delivers a
   * second, live copy of it (#60 — a repeated `text-end` makes the AI SDK
   * report a missing text part). Keeping the store synchronous holds storage
   * and delivery in one turn and lets a SQL failure throw before anything
   * reaches a client. The return type stays awaitable so an override that
   * does real async work is still awaited by the producers — after the
   * broadcast, never before it.
   */
  ...
  private async _broadcastTextEvent(
    ...
    // Vendor divergence: store + broadcast in one turn (#60). Upstream awaits
    // `_storeStreamChunk` here; on `@mcp-b/do-runtime` that await admits a
    // `STREAM_RESUME_ACK`, whose replay would send this chunk and then let the
  ...
  private _settlePreStreamTurn(
    ...
    // Vendor divergence: a turn that persisted its user message but settled
    // without ever starting a stream still leaves a receipt, so the replayed
    // request the client's outbox keeps retrying is answered from the receipt
    // instead of running inference twice (2026-08-14 ledger entry).
  ...
  /**
   * Vendor divergence: empty completed receipt for a turn that produced no
   * stream (2026-08-14 ledger entry).
   *
   * No cleanup alarm is armed any more: 0.23 reclaims finished stream rows at
   * the persist cutover and on the next `ResumableStream.start()`, and the
   * `_cleanupStreamBuffers` schedule this used to arm is gone with it. A
   * receipt therefore lives until this agent starts its next stream rather
   * than for the old ten-minute completed-buffer window — admission reads it
   * (`hasRequest`) before any `start()`, so a replayed request for the turn
   * that just settled is still caught.
   */
  ...
  private async _chatRecoveryProgressMarker(): Promise<number> {
    ...
    // Vendor divergence: upstream runs the seed as a detached
    // `readChatRecoveryProgress(...).then(...)` continuation. `seedProgress`
    // writes `cf_agents_chat_progress`, and on `@mcp-b/do-runtime` a `.then`
    // callback reaches storage without the caller's actor event context — the
    // context is captured at the first `await` of a synchronously-started
    // async function. Starting the same work inline keeps the write inside
    // this evaluation's input lock. Memoization, the retry-on-failed-read
    // (the write is idempotent by `MAX`, so only the read is guarded) and the
    // ordering are upstream's.
    this._progressSeed ??= (async () => {
      let legacy: number;
      try {
        legacy = await readChatRecoveryProgress(this.ctx.storage);
      } catch (error) {
        this._progressSeed = null;
        throw error;
      }
      this._resumableStream.seedProgress(legacy);
    })();
      ...
      for (const line of lines) {
        if (line.startsWith("data: ") && line !== "data: [DONE]") {
          try {
            ...
            // Vendor divergence: store + broadcast in one turn (#60) — see
            // `_storeStreamChunk`. Upstream awaits the store first; on
            // `@mcp-b/do-runtime` that await admits a `STREAM_RESUME_ACK`
            // whose replay carries this chunk, and the broadcast below would
