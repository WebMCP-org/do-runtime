...
describe("VoiceInput — continuous STT pipeline", () => {
  it("delivers the transcriber's final close flush before reporting idle", async () => {
    const { ws } = await connectWS(uniquePath("test-voice-input-agent"));
    try {
      await waitForStatus(ws, "idle");
      sendJSON(ws, { type: "_flush_on_close" });
      await waitForAck(ws, "_flush_on_close");
      sendJSON(ws, { type: "start_call" });
      await waitForStatus(ws, "listening");
      sendJSON(ws, { type: "end_call" });
      await waitForStatus(ws, "idle");
      expect(await getAgentState(ws)).toMatchObject({
        transcripts: ["final transcript on close"],
        callEnd: 1
      });
    } finally {
      ws.close();
