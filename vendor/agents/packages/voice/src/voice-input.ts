...
export interface VoiceInputMixinMembers {
  ...
  /** `error` is non-null when the transcriber fails, including during close(). */
  onCallEnd(
    connection: Connection,
    error: unknown | null
  ): void | Promise<void>;
  onAudioLevel(level: number, connection: Connection): void;
...
export function withVoiceInput<TBase extends AgentLike>(
  ...
  class VoiceInputMixin extends Base {
    ...
    constructor(...args: any[]) {
      ...
      (this as any).onMessage = (
        ...
        if (message instanceof ArrayBuffer) {
          ...
          this.onAudioLevel(computePcm16Rms(message), connection);
          return;
        }
    ...
    onAudioLevel(_level: number, _connection: Connection): void {}
    ...
    async #handleStartupFailure(
      ...
      await this.onCallEnd(connection, error ?? null);
    }
    ...
    async #handleTranscriberFatal(
      ...
      await this.onCallEnd(connection, error);
    }
    ...
    async #handleEndCall(connection: Connection) {
