...
const AGENT_CHAT_REPLAY_MESSAGE_TYPES = new Set<string>([
  ...
  CHAT_MESSAGE_TYPES.MESSAGE_UPDATED,
...
> & {
  ...
  const agentChatReplayRef = useRef<AgentChatReplayBuffer>({
    frames: [],
    subscribers: 0,
    overflowed: false
  });
  ...
  if (replayAddressKeyRef.current !== addressKey) {
    replayAddressKeyRef.current = addressKey;
    agentToolReplayRef.current = { frames: [], drained: false };
    // Only the frames belong to the old address. The chat buffer is cleared in
    // place, never replaced: a mounted `useAgentChat` holds a release closure
    // over THIS object, so a fresh one would both start at zero subscribers
    // while a hook is still listening (every frame delivered live AND stashed,
    // duplicating the next mount's replay) and never see that hook's
    // decrement.
    ...
    onMessage: (message) => {
      if (typeof message.data === "string") {
        ...
        if (
          ...
          AGENT_CHAT_REPLAY_MESSAGE_TYPES.has(parsedMessage.type)
        ) {
          bufferChatReplayFrame(
            agentChatReplayRef.current,
            message.data,
            parsedMessage.type
          );
        }
...
/**
 * Frames the server replays at socket connect can arrive before
 * `useAgentToolEvents` attaches its listener — the listener binds in a
 * passive effect, after paint. A network transport loses that race in
 * practice (the connect round trip outlasts the effect flush), but an
 * in-process transport (a port bridge to a local worker) completes connect
 * and replay first, reliably on a hot remount, and the frames were lost.
 *
 * `useAgent` already parses every string frame in its construction-time
 * `onMessage` — before anything can be delivered — so it stashes
 * `agent-tool-event` frames here until the first subscriber drains them.
 * One-shot: after the first drain the live listener owns delivery (the
 * socket is a stable EventTarget across reconnects), and the client-side
 * `(parent, run, sequence)` dedupe makes the buffered/live overlap safe.
 */
type AgentToolReplayBuffer = { frames: string[]; drained: boolean };
const agentToolReplayBuffers = new WeakMap<object, AgentToolReplayBuffer>();
// Bounds a connection whose app never mounts a subscriber; a full replay of
// retained runs sits far below this.
const AGENT_TOOL_REPLAY_BUFFER_LIMIT = 4096;

// A chat surface can unmount while useAgent keeps its socket alive.
type AgentChatReplayBuffer = {
  frames: string[];
  subscribers: number;
  overflowed: boolean;
};
const agentChatReplayBuffers = new WeakMap<object, AgentChatReplayBuffer>();
const AGENT_CHAT_REPLAY_BUFFER_LIMIT = 4096;

function bufferChatReplayFrame(
  buffer: AgentChatReplayBuffer,
  data: string,
  type: unknown
): void {
  if (buffer.subscribers > 0) return;
  if (buffer.frames.length === AGENT_CHAT_REPLAY_BUFFER_LIMIT) {
    buffer.frames.length = 0;
    buffer.overflowed = true;
  }
  if (buffer.overflowed) {
    // A partial chunk log cannot reconstruct the transcript. Keep only the
    // latest snapshot with no later mutations; hydration and the normal resume
    // handshake reconcile completed and still-active streams on the next mount.
    if (
      type === CHAT_MESSAGE_TYPES.CHAT_MESSAGES ||
      type === CHAT_MESSAGE_TYPES.CHAT_CLEAR
    ) {
      buffer.frames.splice(0, buffer.frames.length, data);
    } else if (
      type === CHAT_MESSAGE_TYPES.USE_CHAT_RESPONSE ||
      type === CHAT_MESSAGE_TYPES.MESSAGE_UPDATED
    ) {
      // Stream completion and tool updates need not send another snapshot.
      // Do not overwrite fresh hydration after the transcript advanced.
      buffer.frames.length = 0;
    }
    return;
  }
  buffer.frames.push(data);
}

/** @internal Buffer a chat frame while no subscriber is mounted. */
export function _bufferAgentChatReplayFrame(agent: object, data: string): void {
  let buffer = agentChatReplayBuffers.get(agent);
  if (!buffer) {
    buffer = { frames: [], subscribers: 0, overflowed: false };
    agentChatReplayBuffers.set(agent, buffer);
  }
  try {
    const message = JSON.parse(data) as { type?: unknown };
    bufferChatReplayFrame(buffer, data, message.type);
  } catch {
    // Only parsed chat frames can be replayed.
  }
}

/** @internal Drain chat frames after the live listener is attached; the returned release must run when the subscriber unmounts. */
export function _drainAgentChatReplayFrames(
  agent: object,
  ...
  buffer.subscribers++;
  for (const data of buffer.frames.splice(0)) onFrame(data);
...
/**
 * INTERNAL. Buffer a raw `agent-tool-event` frame for an agent socket until
 * `useAgentToolEvents` first subscribes to it. `useAgent` calls this for
 * every such frame automatically; a host driving a custom (non-`useAgent`)
 * socket can call it from its own construction-time message hook to get the
 * same no-lost-replay guarantee. No-op once the first subscriber drained.
 */
export function _bufferAgentToolReplayFrame(agent: object, data: string): void {
  let buffer = agentToolReplayBuffers.get(agent);
  if (!buffer) {
    buffer = { frames: [], drained: false };
    agentToolReplayBuffers.set(agent, buffer);
  }
  if (buffer.drained) return;
  if (buffer.frames.length >= AGENT_TOOL_REPLAY_BUFFER_LIMIT) return;
  buffer.frames.push(data);
}

/** Shared head of every dedupe key for one run, so a replay can purge them. */
function agentToolRunKeyPrefix(message: AgentToolEventMessage): string {
  return [message.parentToolCallId ?? "", message.event.runId, ""].join("\0");
}
...
type AgentToolPendingReplay = {
  ...
  enumerated: boolean;
  liveRuns: Set<string>;
  chunks: Map<string, AgentToolEventMessage[]>;
};
...
>(options: {
  ...
  const [owned, setOwned] = useState(() => ({
    agent,
    state: createAgentToolEventState<Part>()
  }));
  // Never expose the previous conversation's rows during the render before
  // its effect is cleaned up and the new source subscribes.
  const state =
    owned.agent === agent ? owned.state : createAgentToolEventState<Part>();
  const sourceRef = useRef(agent);
  sourceRef.current = agent;
  const subscribedSourceRef = useRef(agent);
  const seenRef = useRef(new Set<string>());
  const pendingRef = useRef<AgentToolPendingReplay | null>(null);
  const coverageRef = useRef(new Map<string, Map<string, number>>());

  useEffect(() => {
    // StrictMode restarts this effect without replacing the transport or the
    // projection. Its one-shot early buffer has already drained, so retain
    // the pending replay and dedupe state until the source actually changes.
    if (subscribedSourceRef.current !== agent) {
      subscribedSourceRef.current = agent;
    ...
    const update = (
      apply: (prev: AgentToolEventState<Part>) => AgentToolEventState<Part>
    ) => {
      setOwned((prev) =>
        prev.agent === agent && sourceRef.current === agent
          ? { agent, state: apply(prev.state) }
          : prev
      );
    };
    const onMessage = (event: MessageEvent) => {
      ...
      if (message.event.kind === "collection") {
        ...
        if (collectionEvent.status === "loading") {
          ...
          // A restarted parent may number live revisions from zero again.
          seenRef.current.clear();
        }
        ...
        if (collectionEvent.status === "ready") {
          ...
          for (const runId of pending.chunks.keys()) {
            if (!roster.has(runId)) pending.chunks.delete(runId);
          }
          for (const runId of coverageRef.current.keys()) {
            if (!roster.has(runId) && !pending.liveRuns.has(runId))
              coverageRef.current.delete(runId);
          }
          projectionMessage = {
            ...projectionMessage,
            event: {
              ...collectionEvent,
              runIds: [...new Set([...roster, ...pending.liveRuns])]
            }
          };
      ...
      // A replayed `started` opens a complete, authoritative re-send of the
      // run: the server renumbers replayed frames over stored chunks only,
      // while live numbering also counted non-stored progress frames, so a
      // replayed sequence can equal an already-seen live one. Forget every key
      // seen for the run and let the reducer rebuild it from the replay (its
      // `started` resets the parts) instead of dropping frames — a dropped
      // replayed terminal would leave the row running forever.
      if (
        runMessage.replay &&
        !runMessage.replayId &&
        runMessage.event.kind === "started"
      ) {
        const prefix = agentToolRunKeyPrefix(runMessage);
        for (const seen of seenRef.current) {
          if (seen.startsWith(prefix)) seenRef.current.delete(seen);
        }
        coverageRef.current.delete(runMessage.event.runId);
      } else if (seenRef.current.has(key)) {
        return;
      }
      seenRef.current.add(key);
      const pending = pendingRef.current;
      const runId = runMessage.event.runId;
      if (!runMessage.replay) {
        // Inspection and the parent's live tail are independent RPC paths.
        // Covered chunks can arrive after hydration with a new parent revision;
        ...
        if (
          ...
          runMessage.event.cursor.sequence <=
            (coverageRef.current
              .get(runId)
              ?.get(runMessage.event.cursor.epoch) ?? -1)
        )
          return;
        pending?.liveRuns.add(runId);
        if (
          pending &&
          runMessage.event.kind === "chunk" &&
          (!pending.enumerated || pending.chunks.has(runId))
        ) {
          const chunks = pending.chunks.get(runId) ?? [];
          chunks.push(runMessage);
        ...
        if (!runMessage.event.snapshot) {
          ...
          if (snapshot.replay?.cursor) {
            coverage.set(
              snapshot.replay.cursor.epoch,
              Math.max(
                coverage.get(snapshot.replay.cursor.epoch) ?? -1,
                snapshot.replay.cursor.sequence - 1
              )
            );
          }
          for (const chunk of snapshot.replay?.chunks ?? []) {
            if (chunk.cursor) {
              coverage.set(
                chunk.cursor.epoch,
                Math.max(
                  coverage.get(chunk.cursor.epoch) ?? -1,
                  chunk.cursor.sequence
                )
              );
            }
          }
          const uncovered = buffered.filter((frame) => {
          ...
          // contains. Preserve live state rather than clear it or duplicate it.
          const unordered = buffered.length > 0 && !snapshot.replay?.cursor;
          if (!unordered) coverageRef.current.set(runId, coverage);
          update((prev) => {
            ...
            if (unordered)
              return {
                ...
                collection: {
                  ...
                  stale: true,
                  error:
                    "The child could not order its saved transcript with live updates. Reconnect to refresh."
                }
              };
            for (const frame of uncovered)
              next = applyAgentToolEvent(next, frame);
            return next;
          });
          return;
        }
      }
      update((prev) => applyAgentToolEvent<Part>(prev, runMessage));
    };
    ...
    const onClose = () => {
      ...
      update((prev) => ({
        ...prev,
        collection: { status: "loading", stale: true }
      }));
    };
    const onError = () => {
      if (sourceRef.current !== agent) return;
      update((prev) => ({
        ...
        collection: {
          status: "error",
          stale: true,
          error: "Could not connect to delegated work."
        }
      }));
    };
    ...
    // Drain frames that arrived before this listener existed (the server
    // replays retained runs at socket connect, which an in-process
    // transport completes before this passive effect — see
    // agentToolReplayBuffers). Listener first, then drain: a frame landing
    // mid-drain is deduped by `seenRef`, not lost.
    const replay = agentToolReplayBuffers.get(agent);
    if (replay) {
      replay.drained = true;
      for (const data of replay.frames.splice(0)) {
  ...
  const resetLocalState = useCallback(() => {
    ...
    pendingRef.current = null;
    coverageRef.current.clear();
    setOwned({ agent, state: createAgentToolEventState<Part>() });
  }, [agent]);
