...
export type AgentToolFailure = {
  ...
  /** Child execution settlement; see {@link RunAgentToolResult.childStillRunning}. */
 ...
 * are retained by default. `at` is the original signal time; legacy rows that
 * did not retain it fall back to their last recorded activity time.
 */
...
export type RunAgentToolResult<Output = unknown> = {
  ...
  reason?: AgentToolInterruptedReason;
  /**
   * For `interrupted` or `aborted`, `false` confirms no remaining owned child
   * execution; `true` means execution may continue, and absent means unknown.
   * Settlement retains the facet and history, does not undo external effects,
   * and does not make an intentionally aborted run retryable.
   */
  childStillRunning?: boolean;
};

/** Outcome of one cancellation command, separate from the retained run status. */
export type AgentToolCancellationResult =
  | { runId: string; status: "fulfilled" }
  | {
      runId: string;
      status: "rejected";
      /** Diagnostic text may be empty; use `status` to identify failure. */
      error: string;
    };

/** Retained parent-owned metadata and the accepted result for one run. */
export type AgentToolRunSnapshot<Output = unknown> = AgentToolRunInfo &
  Omit<RunAgentToolResult<Output>, "runId" | "agentType" | "status">;

/** Whether execution is active or has not yet been confirmed stopped. */
export function agentToolRunMayExecute(
  run: Pick<AgentToolRunSnapshot, "status" | "childStillRunning">
): boolean {
  return (
    run.status === "starting" ||
    run.status === "running" ||
    ((run.status === "interrupted" || run.status === "aborted") &&
      run.childStillRunning !== false)
  );
}

export type ChatCapableAgentClass<T extends Agent = Agent> = SubAgentClass<T>;

/** A position in one live producer or persisted stream; epochs are opaque. */
export type AgentToolReplayCursor = {
  epoch: string;
  sequence: number;
};
...
export type AgentToolRunInspection<Output = unknown> = {
  ...
  /**
   * Optional atomic transcript/progress snapshot requested by a reconnecting
   * projection. `cursor` is the exclusive next live position; chunk cursors
   * identify included persisted stream positions. No history is re-emitted.
   */
  replay?: {
    chunks: AgentToolStoredChunk[];
    /** Absent on a legacy child that cannot provide an atomic live boundary. */
    cursor?: AgentToolReplayCursor;
  };
};
...
export type AgentToolChildAdapter<Input = unknown, Output = unknown> = {
  ...
  /** A false acknowledgment confirms settlement; a void reply leaves it unknown. */
  cancelAgentToolRun(
    runId: string,
    reason?: unknown
  ...
  tailAgentToolRun?(
    runId: string,
    options?: {
      /** Legacy offset in the stored transcript. */
      afterSequence?: number;
      /** Prefer the source-scoped offset when a stream may have restarted. */
...
export type AgentToolEvent =
  ...
  | {
      kind: "snapshot";
      runId: string;
      /** Lifecycle remains owned by the parent, not the child's inspection. */
      run: AgentToolRunSnapshot;
      snapshot?: Pick<
        AgentToolRunInspection,
        "progress" | "milestones" | "replay"
      >;
    }
  | {
      kind: "finished";
...
export type AgentToolEventMessage = {
  ...
  sequence: number;
  replay?: true;
  /** Scope renumbered replay frames to one connection's replay. */
  replayId?: string;
  /** Parent-instance live event order, independent of per-run tail sequence. */
  revision?: number;
  event: AgentToolEvent;
};

export type AgentToolCollectionState = {
  status: "loading" | "ready" | "error";
  /** Last-known rows may be stale while reconnecting or after a failed read. */
  stale: boolean;
  error?: string;
};

/** Collection metadata uses the existing agent-tool-event envelope. */
export type AgentToolCollectionMessage = {
  type: "agent-tool-event";
  replay: true;
  replayId: string;
  sequence: number;
  event:
    ...
    | {
        kind: "collection";
        status: "ready";
        /** Complete roster, including [] when no runs are retained. */
        runIds: string[];
      }
    | {
        kind: "collection";
...
> = {
  ...
  error?: string;
  /**
   * The cause is present for `interrupted`.
   */
  reason?: AgentToolInterruptedReason;
  /** Child execution settlement; see {@link RunAgentToolResult.childStillRunning}. */
  childStillRunning?: boolean;
