...
> extends DurableObject<Env> {
  ...
  readonly _state: State<unknown> = new State<unknown>({
    ...
    // Vendor divergence: lossless hydration. Stored state decodes through
    // `migratePersistedState` with no corrupt-row fallback (StateOptions).
    migratePersistedState: (state) => this.migratePersistedState(state)
  });
  ...
  get state(): TState {
    ...
    // A corrupt row throws (vendor divergence: lossless hydration).
   ...
   */
  protected migratePersistedState(state: unknown): TState {
