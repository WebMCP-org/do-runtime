...
export { camelCaseToKebabCase, isInternalJsStubProp } from "./utils";
...
import { raceWithSignal } from "./mcp/abort";
...
export type MCPServer = {
  ...
  state: MCPConnectionState | "not-connected";
...
> extends DurableObject<Env> {
  ...
  /** @internal Detached terminals withheld until their matching start event. */
  private _agentToolSuppressedTerminalEvents = new Set<string>();
  ...
  get state(): State {
    ...
    const result = this.sql<{ state: string | null }>`
  ...
  *getConnections<TState = unknown>(
  ...
  private async _cf_forwardSubAgentWebSocketConnect(
    ...
    // A gate refusal consumed the connect by closing the bridge. It is not a
    // request for this agent, so do not fall through to onConnect where a
    // chat wrapper could try to forward the same request again without a gate.
  ...
  private async _cf_forwardSubAgentWebSocketMessage(
    ...
    // requireRegistered: resolving is what registers a sub-agent, so a frame
    // arriving on a stale connection after `deleteSubAgent` would otherwise
    // recreate the child it targets. Consume the frame and close the
    // connection instead — the child is gone, falling through would hand a
    // child-protocol frame to the parent, and leaving the socket open would
    // strand the sender's call pending forever on a connection that still
    // reports itself connected. Fresh connects (which legitimately
    // lazy-create) don't pass this flag.
    const routed = await this._cf_resolveSubAgentConnection(
      connection,
      undefined,
      { gate: false, requireRegistered: true }
    );
  ...
  private async _cf_forwardSubAgentWebSocketClose(
    ...
    // See _cf_forwardSubAgentWebSocketMessage: the close of a connection to a
    // deleted child must not resurrect it.
    const routed = await this._cf_resolveSubAgentConnection(
      connection,
      undefined,
      { gate: false, requireRegistered: true }
  ...
  > {
    ...
    if (
      ...
      !this.hasSubAgent(match.childClass, match.childName)
    ) {
      // The caller's connection targets a child that has been deleted since
      // it connected. `_cf_resolveSubAgent` below is create-on-access — it
      // would re-register the child — so refuse to resolve rather than let a
      // stale frame resurrect a deleted agent.
      return "deleted";
    }
  ...
  private _cf_createSubAgentBridgeConnection(
    ...
    if (stored) {
      ...
      if (stored.connection) {
        // A virtual connection is re-pointed at the latest meta, not recreated.
        const mutable = stored.connection as unknown as {
          uri: string | null;
          tags: string[];
        };
        mutable.uri = meta.uri;
        mutable.tags = meta.tags;
  ...
  private _cf_storeVirtualSubAgentConnection(connection: Connection): void {
    ...
    // A close can remove the record while onConnect awaits user code.
    if (!stored) return;
  ...
  /** Best-effort notification of a retained lifecycle or liveness change. */
  async onAgentToolRunChanged(_run: AgentToolRunSnapshot): Promise<void> {}
  ...
  async runAgentTool<Input = unknown, Output = unknown>(
    ...
    if (options.detached || !signal) {
      return this._runAgentTool<Input, Output>(cls, { ...options, runId });
    }

    // Cover initial admission and reattachment. Cancellation records the parent
    // decision before crossing into the child, so later admission sees it too.
    let cancellation: Promise<void> | undefined;
    const onAbort = () => {
      cancellation ??= this.cancelAgentTool(runId, signal.reason);
      // A public cancel command reports delivery failure. Abort listeners must
      // also observe the rejection when their dispatching turn has stopped.
      void cancellation.catch(() => {});
    };
    signal.addEventListener("abort", onAbort, { once: true });
    if (signal.aborted) onAbort();
    try {
      const result = await this._runAgentTool<Input, Output>(cls, {
        ...options,
        runId
      });
      if (cancellation) {
        ...
        if (row && this._isAgentToolRowHardTerminal(row.status)) {
          return this._resultFromAgentToolRow<Output>(row);
        }
      }
      return result;
    } finally {
  ...
  private async _runAgentTool<Input, Output>(
    cls: ChatCapableAgentClass,
    options: RunAgentToolOptions<Input> & { runId: string }
  ): Promise<RunAgentToolResult<Output> | DetachedRunAgentToolResult> {
    ...
    if (existing) {
      ...
      if (
        ...
        if (existing.status === "completed" && existing.output_json == null) {
          try {
            ...
            const inspection = await raceWithSignal(
              Promise.resolve(adapter.inspectAgentToolRun(runId)),
              options.signal
            );
      ...
      try {
        const child = await raceWithSignal(
          this.subAgent(cls as SubAgentClass<Agent>, runId),
          options.signal
        );
        const adapter = this._asAgentToolChildAdapter<Input, Output>(child);
        ...
        if (reattach.result) {
          const finished = await this._finishAgentToolRun(
      ...
      const cancelled = this._readAgentToolRun(runId);
      if (cancelled?.status === "aborted") {
        return (
          await this._finishAgentToolRun(
            this._agentToolRunInfoFromRow(cancelled),
            this._resultFromAgentToolRow<Output>(cancelled),
            { sequence: 1 }
          )
        ).result;
      }
    ...
    if (
      (detached || !options.signal?.aborted) &&
      this._activeAgentToolRunCount() >= this.maxConcurrentAgentTools
    ) {
      ...
      `;
      this._broadcastAgentToolSnapshot(runId);
    ...
    if (!detached && options.signal?.aborted) {
      await this.cancelAgentTool(runId, options.signal.reason);
      return this._resultFromAgentToolRow<Output>(
        this._readAgentToolRun(runId)!
      );
    }
    this._broadcastAgentToolSnapshot(runId);
    let adapter: AgentToolChildAdapter<Input, Output>;
    let childStart: AgentToolRunInspection<Output>;
    let startedPublished = false;
    let terminalEventWasSuppressed = false;
    // An awaited cancel can land across either of the awaits below. Settle on
    // the durable fence instead of publishing `started` for a stopped run.
    const settleIfCancelled = async (): Promise<
      RunAgentToolResult<Output> | undefined
    > => {
      const row = this._readAgentToolRun(runId);
      if (detached || row?.status !== "aborted") return undefined;
      return (
        await this._finishAgentToolRun(
          runInfo,
          this._resultFromAgentToolRow<Output>(row)
        )
      ).result;
    };
    try {
      await this.onAgentToolStart(runInfo);
      const cancelledBeforeResolve = await settleIfCancelled();
      if (cancelledBeforeResolve) return cancelledBeforeResolve;
      const child = await this.subAgent(cls as SubAgentClass<Agent>, runId);
      const cancelledBeforeStart = await settleIfCancelled();
      if (cancelledBeforeStart) return cancelledBeforeStart;
      terminalEventWasSuppressed =
        detached !== null &&
        this._agentToolSuppressedTerminalEvents.delete(runId);
      ...
      if (detached) {
        if (terminalEventWasSuppressed && startedPublished) {
          this._broadcastAgentToolTerminal(runInfo.parentToolCallId, 1, result);
        }
        await this._deliverDetachedTerminal(runId, "finish", result, {
          sequence: 1,
          projectEvent: startedPublished && !terminalEventWasSuppressed
        });
        return { runId, agentType, status: "running" };
      }
      return (
        await this._finishAgentToolRun(runInfo, result, {
          ...(startedPublished ? { sequence: 1 } : {})
        })
      ).result;
    }
    // An explicit cancel can land after the parent row is published but before
    // the child has durably admitted its run. The parent's terminal row is the
    // cancellation fence: if the first child-side cancel raced too early, tear
    // down the now-admitted child before tailing it.
    const cancelled = this._readAgentToolRun(runId);
    if (cancelled?.status === "aborted") {
      try {
        if (detached) {
          await adapter.cancelAgentToolRun(
            runId,
            cancelled.error_message ?? undefined
          );
        } else {
          await this._awaitAgentToolCancellation(
            runId,
            adapter.cancelAgentToolRun(
              runId,
              cancelled.error_message ?? undefined
            )
          );
        ...
        // Keep the accepted abort; failed delivery remains visible and retryable.
      }
      const result = this._resultFromAgentToolRow<Output>(
        this._readAgentToolRun(runId) ?? cancelled
      );
      if (detached) {
        if (terminalEventWasSuppressed) {
          this._broadcastAgentToolTerminal(runInfo.parentToolCallId, 1, result);
        }
        await this._deliverDetachedTerminal(runId, "finish", result, {
          sequence: 1,
          ...(terminalEventWasSuppressed ? { projectEvent: false } : {})
        });
        return { runId, agentType, status: "running" };
      }
      return (await this._finishAgentToolRun(runInfo, result, { sequence: 1 }))
        .result;
    }
    ...
    try {
      if (adapter.tailAgentToolRun) {
        ...
        const stream = await raceWithSignal(
          Promise.resolve(
            adapter.tailAgentToolRun(runId, { afterSequence: -1 })
          ),
          options.signal
        );
        sequence = (
        ...
        options.signal?.throwIfAborted();
        const chunks = await raceWithSignal(
          Promise.resolve(adapter.getAgentToolChunks(runId)),
          options.signal
        );
      ...
      if (options.signal?.aborted) {
        ...
        return (await this._finishAgentToolRun(runInfo, result, { sequence }))
          .result;
      }
  ...
  /**
   * Cancel an agent-tool run by id. A failed or unconfirmed awaited child
   * cancellation can be retried without replacing its durable aborted outcome.
   * Detached runs deliver through the guarded ledger so a wired
   * `onFinish` fires once with `status: "aborted"`; awaited runs persist the
   * cancellation fence but leave terminal observation to the awaiting/recovery
   * path, avoiding duplicate finish hooks.
   */
  async cancelAgentTool(runId: string, reason?: unknown): Promise<void> {
    ...
    if (
      ...
      row.status === "error" ||
      (row.status === "aborted" &&
        (isDetached || row.child_still_running === 0))
    ) {
      return;
    }
    const startedWasPublished = row.status !== "starting";
    const message =
      reason instanceof Error
        ? reason.message
        : String(reason ?? "cancelled by parent");
    const result: RunAgentToolResult = {
      runId,
      agentType: row.agent_type,
      status: "aborted",
      error: message,
      ...(!isDetached ? { childStillRunning: true } : {})
    };
    // Publish the cancellation before crossing into the child. A start and a
    // cancel are separate Agent invocations, so the child call can legitimately
    // arrive before its launch row exists; runAgentTool observes this durable
    // fence immediately after child admission and retries the teardown there.
    this._updateAgentToolTerminal(runId, result);
    const suppressTerminalEvent = isDetached && !startedWasPublished;
    if (suppressTerminalEvent) {
      this._agentToolSuppressedTerminalEvents.add(runId);
    }
    if (!isDetached) this._broadcastAgentToolSnapshot(runId);
    if (this.hasSubAgent(row.agent_type, runId)) {
      const cancellation = (async () => {
        const child = await this._cf_resolveSubAgent(row.agent_type, runId);
        return this._asAgentToolChildAdapter(child).cancelAgentToolRun(
          runId,
          reason
        );
      })();
      if (isDetached) {
        try {
          ...
          // Keep the detached delivery contract; awaited cancellation below
          // exposes unconfirmed delivery to its caller instead.
        }
      } else {
        await this._awaitAgentToolCancellation(runId, cancellation);
      }
    } else if (!isDetached) {
      // The parent decision prevents a pending startup from admitting a child.
      this._setAgentToolChildLiveness(runId, false);
    }
    // A detached cancel can race the runner before it has admitted a child and
    // published `started`. Deliver hooks durably now, but defer the live event
    // to that runner; terminal-before-start is ignored by the client reducer,
    // while a synthetic start here would expose a child that does not exist.
  ...
  private async _awaitAgentToolCancellation(
    runId: string,
    cancellation: ReturnType<AgentToolChildAdapter["cancelAgentToolRun"]>
  ): Promise<void> {
    const settled = (async () => {
      const acknowledgement = await cancellation;
      this._setAgentToolChildLiveness(
        runId,
        acknowledgement?.childStillRunning
      );
      if (acknowledgement?.childStillRunning !== false) {
        throw new Error(`Agent tool cancellation was not confirmed: ${runId}`);
      }
    })();
    // A late acknowledgement can still settle retained liveness after this
    // bounded command expires; no separate cancellation job is created.
    this.ctx.waitUntil(settled.catch(() => {}));
    await raceWithSignal(
      settled,
      AbortSignal.timeout(DEFAULT_AGENT_TOOL_RECOVERY_TIMEOUT_MS)
    );
  }

  /** Read retained parent records without creating or waking child facets. */
  listAgentToolRuns(): AgentToolRunSnapshot[] {
    return this._listAgentToolRunRows().map((row) =>
      this._agentToolRunSnapshotFromRow(row)
    );
  }

  /** Read one retained run; an unknown id never creates a child. */
  inspectAgentTool(runId: string): AgentToolRunSnapshot | null {
    const row = this._readAgentToolRun(runId);
    return row ? this._agentToolRunSnapshotFromRow(row) : null;
  }

  /**
   * Cancel every currently unresolved child. All parent decisions are written
   * before awaiting child responses; one failed delivery cannot skip siblings.
   * A successful command is not a promise to roll back external side effects.
   */
  async cancelAgentTools(
    reason?: unknown
  ): Promise<AgentToolCancellationResult[]> {
    const runs = this._listAgentToolRunRows().filter((row) =>
      this._agentToolRunCouldExecute(row)
    );
    return Promise.all(
      runs.map(async (row): Promise<AgentToolCancellationResult> => {
        try {
          await this.cancelAgentTool(row.run_id, reason);
          return { runId: row.run_id, status: "fulfilled" };
        } catch (error) {
          return {
            runId: row.run_id,
            status: "rejected",
  ...
  private _setAgentToolChildLiveness(
    runId: string,
    childStillRunning: boolean | undefined
  ): void {
    // A shutdown acknowledgement can precede the parent's interrupted seal
    // while its replay read is still pending.
    const value =
      childStillRunning === undefined ? null : childStillRunning ? 1 : 0;
    this.sql`
      UPDATE cf_agent_tool_runs
      SET child_still_running = CASE
        WHEN child_still_running = 0 THEN 0 ELSE ${value} END
      WHERE run_id = ${runId} AND status NOT IN ('completed', 'error')
    `;
    this._broadcastAgentToolSnapshot(runId);
  }

  private _broadcastAgentToolSnapshot(runId: string): void {
    const run = this.inspectAgentTool(runId);
    if (!run) return;
    this._broadcastAgentToolEvent(run.parentToolCallId, 0, {
      kind: "snapshot",
      runId,
      run
    });
    this.ctx.waitUntil(
      (async () => {
        try {
          // Enter the hook while this actor owns its input context. Deferring
          // the invocation to an unowned Promise callback loses that context.
          await this.onAgentToolRunChanged(run);
        } catch (error) {
          console.error(
            `[agents] onAgentToolRunChanged hook threw for run ${runId}:`,
            error instanceof Error ? error.message : String(error)
          );
        }
      })()
  ...
  private async _deliverDetachedTerminal<Output>(
    ...
    try {
      await this._runDetachedDelivery(invoke, {
        serialize: options?.serialize
      });
    } catch (error) {
      // A claimed delivery stays retryable until its lease expires. Explicit
      // cancellation and admission failure can reach this funnel before the
      // normal post-admission backbone is armed, so ensure the durable retry
      // exists at the shared failure boundary.
      try {
        await this._armDetachedBackbone();
      } catch {
        // The original delivery failure remains the useful error. Startup
        // recovery also re-arms every outstanding detached finish.
      }
      throw error;
    }
  ...
  async _cfDetachedReconcileTick(
    ...
    for (const row of rows) {
      ...
      // Cancellation or admission failure can terminalize the parent before a
      // child exists. The parent row is already authoritative in that case;
      // resolve it first so recovery neither mints an empty child nor later
      // misclassifies the terminal as a budget give-up.
      if (this._isAgentToolRowHardTerminal(row.status)) {
        await this._deliverDetachedTerminal(
          runId,
          "finish",
          this._resultFromAgentToolRow(row),
          {
            serialize: true,
            projectEvent: this.hasSubAgent(row.agent_type, runId)
          },
  ...
  private _activeAgentToolRunCount(): number {
    const rows = this.sql<{ n: number }>`
      ...
      WHERE status IN ('starting', 'running')
        OR (detached = 0 AND status IN ('interrupted', 'aborted')
            AND child_still_running IS NOT 0)
    `;
    return rows[0]?.n ?? 0;
  }

  private _agentToolRunCouldExecute(row: AgentToolRunStorageRow): boolean {
    // Detached lifecycle delivery has its own terminal contract; awaited
    // children share the same conservative predicate as their UI projection.
    if (row.detached === 1) {
      return row.status === "starting" || row.status === "running";
    }
    return agentToolRunMayExecute({
      status: row.status,
      childStillRunning:
  ...
  private _listAgentToolRunRows(): AgentToolRunStorageRow[] {
    return this.sql<AgentToolRunStorageRow>`
      SELECT run_id, parent_tool_call_id, agent_type, input_preview, status,
             summary, output_json, error_message, interrupted_reason,
             child_still_running, display_metadata, display_order,
             started_at, completed_at, detached, detached_on_finish,
  ...
  private _agentToolRunSnapshotFromRow(
  ...
  /**
   * Reconstruct an interruption cause and unresolved child liveness for both
   * interrupted and aborted rows. Cancellation is a durable parent decision;
   * only a child shutdown acknowledgement proves it no longer owns work.
   */
  private _agentToolInterruptedExtrasFromRow(row: {
    ...
    if (row.status !== "interrupted" && row.status !== "aborted") return {};
    return {
      ...(row.status === "interrupted" && row.interrupted_reason !== null
  ...
  private async _finishAgentToolRun<Output>(
    ...
    result: RunAgentToolResult<Output>;
    finish?: DeferredAgentToolFinish;
  }> {
    this._updateAgentToolTerminal(run.runId, result, options?.completedAt);
    const row = this._readAgentToolRun(run.runId);
    const accepted = row ? this._resultFromAgentToolRow<Output>(row) : result;
    const completedAt = row?.completed_at ?? options?.completedAt ?? Date.now();
    this._broadcastAgentToolSnapshot(run.runId);
    ...
    const finish = () =>
      this.onAgentToolFinish(
        { ...run, status: accepted.status, completedAt },
        accepted
      );
    if (options?.deferFinishHook) return { result: accepted, finish };
    await finish();
  ...
  private _updateAgentToolTerminal<Output>(
    ...
    // saw. A hard terminal clears the stale cause, but confirmed child shutdown
    // is monotonic: a pending replay can carry an older true/unknown value.
    ...
    this.sql`
      ...
      SET status = ${result.status},
          ...
          child_still_running = CASE
            WHEN child_still_running = 0 THEN 0 ELSE ${childStillRunning} END,
          completed_at = ${completedAt}
      WHERE run_id = ${runId}
  ...
  private _agentToolEventRevision = 0;
  private _agentToolReplayIds = new Map<string, string>();
    ...
    const message: AgentToolEventMessage = {
      ...
      ...(replay
        ? {
            replay,
            ...(connection && {
              replayId: this._agentToolReplayIds.get(connection.id)
            })
          }
        : { revision: ++this._agentToolEventRevision })
    };
   ...
   * Returns whether owned child execution was confirmed stopped. The facet
   * and history remain retained; external effects are not rolled back.
   */
  ...
  private async _reattachAgentToolRunToTerminal<Output>(
    ...
    const collectTerminal = async (
      ...
      try {
        signal?.throwIfAborted();
        inspection = await raceWithSignal(
          Promise.resolve(adapter.inspectAgentToolRun(row.run_id)),
          signal
        );
      } catch {
    ...
    const waitSignal = signal
      ? AbortSignal.any([signal, ceilingController.signal])
      ...
      while (!waitSignal.aborted) {
        ...
        try {
          ...
          const existing = await raceWithSignal(
            Promise.resolve(adapter.getAgentToolChunks(row.run_id)),
            waitSignal
          );
        ...
        try {
          ...
          const stream = await raceWithSignal(
            Promise.resolve(
              adapter.tailAgentToolRun(row.run_id, {
                afterSequence,
                afterCursor
              })
            ),
  ...
  private async _replayAgentToolRuns(connection: Connection): Promise<void> {
    ...
    this._agentToolReplayIds.set(connection.id, replayId);
    const current = () =>
      this._agentToolReplayIds.get(connection.id) === replayId;
    let collectionSequence = 0;
    const collection = (event: AgentToolCollectionMessage["event"]) => {
      if (!current()) return;
      connection.send(
        JSON.stringify({
          type: "agent-tool-event",
    ...
    try {
      collection({ kind: "collection", status: "loading" });
      const rows = this._listAgentToolRunRows();

      // Publish the whole roster synchronously. A slow or unavailable child's
      // transcript must not hide another run or make an empty roster ambiguous.
      for (const row of rows) {
        this._broadcastAgentToolEvent(
          row.parent_tool_call_id ?? undefined,
          0,
          {
            kind: "snapshot",
            runId: row.run_id,
            run: this._agentToolRunSnapshotFromRow(row)
          },
          true,
          connection
        );
          ...
          try {
            ...
            const details = await Promise.race([
              ...
              new Promise<never>((_, reject) => {
                timer = setTimeout(
                  () => reject(new Error("child snapshot timed out")),
            ...
            this._broadcastAgentToolEvent(
              ...
              2,
              {
                kind: "snapshot",
                runId: row.run_id,
                run: this._agentToolRunSnapshotFromRow(retained),
                snapshot: details
              },
              true,
              connection
      ...
      if (current()) this._agentToolReplayIds.delete(connection.id);
    }
  }
  ...
  private async _reconcileAgentToolRuns(options?: {
    ...
    const finalizeRow = async (
      ...
      if (finished.finish) {
        deferredFinishes.push(finished.finish);
      }
    };
    ...
    for (const row of recoveryRows) {
      ...
      if (row.status === "aborted") {
        // An earlier stop may have outlived its parent's delivery attempt.
        // Re-drive only that cancellation, never inspect it into completion or
        // restart the task. The normal startup budget still bounds this work.
        if (remainingMs > 0) {
          try {
            await raceWithSignal(
              this.cancelAgentTool(row.run_id, row.error_message),
              Number.isFinite(remainingMs)
                ? AbortSignal.timeout(Math.ceil(remainingMs))
                : undefined
            );
          } catch {
            // Retained unconfirmed liveness remains eligible for retry.
          }
        }
        continue;
      }
  ...
  /** Wait for startup reconciliation of parent-side agent-tool runs. */
  protected async _waitForAgentToolRunRecovery(): Promise<void> {
    await this._agentToolRunRecoveryPromise;
  }
