  ...
  *getVirtualConnections(tag?: string): Iterable<Connection> {
  ...
  async handleWebSocketConnect(
    ...
    try {
      ...
      // Vendor divergence: a failed setup must not leave a half-registered
      // connection record behind for later lookups to find.
  ...
  async handleWebSocketClose(
    ...
    try {
      ...
      // Vendor divergence: a throwing onClose must still retire the record.
      this.#virtualConnections.delete(meta.id);
    }
  }
  ...
  storeVirtualConnection(connection: Connection): void {
    ...
    // Vendor divergence: a close can remove the record while onConnect awaits
    // user code. A completed connect hook must not recreate it.
    if (!stored) return;
