...
/**
 * Project a terminal agent-tool lifecycle result into the value returned to a
 * model or orchestration caller. `agentTool()` uses it, and callers that drive
 * `Agent.runAgentTool()` directly share the same success/failure semantics.
 */
export async function projectAgentToolResult<Output = unknown>(
  result: RunAgentToolResult<Output>,
  outputSchema?: AgentToolOutputSchema<Output>
): Promise<string | Output | AgentToolFailure> {
  if (result.status === "completed") {
    if (outputSchema) {
      ...
      return validateOutput(outputSchema, result.output);
    }
...
export function agentTool<Input = unknown, Output = unknown>(
  ...
  return createTool<Input, string | Output | AgentToolFailure>({
    ...
    execute: async (input: Input, executeOptions?: ToolExecutionOptions) => {
      ...
      return projectAgentToolResult(result, options.outputSchema);
    }
  });
}

export type { AgentToolFactoryOptions, AgentToolOutputSchema };
export type {
