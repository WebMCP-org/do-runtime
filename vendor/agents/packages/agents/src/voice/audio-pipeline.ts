...
export function runBackground(
  ...
  // Capture actor identity before deferring, preserving upstream microtask order.
  void (async () => {
...
export class AudioConnectionManager {
  ...
  /** End a client-requested call after its transcriber flushes final text. */
  async endCall(connectionId: string): Promise<void> {
    this.abortPipeline(connectionId);
    this.#audioBuffers.delete(connectionId);
