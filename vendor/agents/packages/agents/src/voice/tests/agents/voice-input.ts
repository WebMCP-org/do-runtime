...
class TestTranscriberSession implements TranscriberSession {
  ...
  constructor(
    ...
    utteranceThreshold = 20000,
    private readonly flushOnClose = false
  ) {
  ...
  async waitUntilClosed(): Promise<void> {
    ...
    if (this.flushOnClose) this.#onUtterance?.("final transcript on close");
  }
}
    ...
    if (typeof message === "string") {
      ...
      switch (parsed.type) {
        ...
        case "_set_on_call_start_throw":
          ...
          break;
        case "_flush_on_close":
          this.transcriber.flushOnClose = true;
          connection.send(
            JSON.stringify({ type: "_ack", command: parsed.type })
          );
          break;
