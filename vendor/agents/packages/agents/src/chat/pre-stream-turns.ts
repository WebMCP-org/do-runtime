...
> {
  ...
  /** Accepted request ids whose incoming messages are durably persisted. */
  private readonly _persisted = new Set<string>();
  ...
  settle(requestId: string): boolean {
    ...
    this._persisted.delete(requestId);
  ...
  /** Whether this live owner has already accepted the request id. */
  hasRequest(requestId: string): boolean {
    return this._accepted.has(requestId);
  }

  /** Whether acceptance reached the host's incoming-message persistence seam. */
  isPersisted(requestId: string): boolean {
    return this._persisted.has(requestId);
  }
  ...
  park(connection: TConnection, probeId?: string): boolean {
    ...
    this._sendPending(connection, this._latestRequestId, probeId);
    return true;
  }

  /**
   * Park a replay of a request already owned by this pre-stream window. The
   * frame names the duplicate request, but is a durable receipt only after
   * {@link markPersisted} adds `persisted: true`.
   */
  parkDuplicate(connection: TConnection, requestId: string): boolean {
    if (!this.hasRequest(requestId)) return false;
    this.awaitingConnections.set(connection.id, connection);
    this._sendPending(connection, requestId);
    return true;
  }

  /**
   * Mark the persistence boundary and acknowledge the originating connection
   * plus any duplicate/reconnect connections already parked on this turn.
   */
  markPersisted(requestId: string, origin: TConnection): void {
    if (!this.hasRequest(requestId)) return;
    this._persisted.add(requestId);
    this._sendPending(origin, requestId);
    for (const connection of this.awaitingConnections.values()) {
      if (connection.id !== origin.id) {
        this._sendPending(connection, requestId);
      }
    }
  }
  ...
  private _sendPending(
    ...
    sendIfOpen(
      ...
      JSON.stringify({
        ...
        ...(requestId && this.isPersisted(requestId)
          ? { persisted: true }
          : {}),
        ...(probeId ? { probeId } : {})
      })
