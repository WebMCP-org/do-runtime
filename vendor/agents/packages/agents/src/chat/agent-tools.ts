...
function readAgentToolProgressChunk(
  ...
  const data =
    (chunk as { data?: AgentToolProgress & { at?: number } }).data ?? {};
  return {
...
function applyToRun<Part extends AgentToolRunPart>(
  ...
  switch (event.kind) {
    case "snapshot": {
      const run = event.run;
      let next: AgentToolRunState<Part> = {
        ...seeded,
        runId: run.runId,
        ...
        parts: seeded?.parts ?? [],
        subAgent: { agent: run.agentType, name: run.runId }
      };
      if (event.snapshot) {
        const { replay, progress, milestones } = event.snapshot;
        if (replay) {
          next = {
            ...next,
            parts: [],
          ...
          for (const chunk of replay.chunks) {
            next =
              applyToRun(next, {
                ...message,
                event: { kind: "chunk", runId: run.runId, body: chunk.body }
              }) ?? next;
          }
        }
        next = {
          ...
          milestones:
            milestones?.reduce<AgentToolMilestone[]>(
              (all, milestone) => mergeMilestone(all, milestone),
              next.milestones ?? []
            ) ?? next.milestones
        };
      }
    ...
    case "started": {
      // A replayed `started` heads a complete re-send of the run's stored
      // chunks (renumbered), so the parts rebuild from it; a live re-`started`
      // (a buffered/live overlap) keeps what already streamed.
      const parts = message.replay ? [] : (seeded?.parts ?? []);
      if (
        ...
        // A settled run is replayed too. Let its chunks rebuild, but never let
        // a `started` — replayed or stray — resurrect the terminal.
        return { ...seeded, parts };
      }
      return {
        ...seeded,
    ...
    case "chunk": {
      ...
      // The dedicated milestone list and the transcript must agree.
      if (
        milestone &&
        seeded.milestones?.some(
          (existing) => existing.sequence === milestone.sequence
        )
      ) {
        return seeded;
      }
...
>(): AgentToolEventState<Part> {
  return {
    ...
    runsByToolCallId: {},
    unboundRuns: [],
    collection: { status: "loading", stale: false }
  };
}
...
>(
  ...
  message: AgentToolProjectionMessage
): AgentToolEventState<Part> {
  ...
  if (message.event.kind === "collection") {
    ...
    if (event.status === "ready") {
      const runIds = new Set(event.runIds);
      runsById = Object.fromEntries(
        Object.entries(runsById).filter(([id]) => runIds.has(id))
      );
    }
    return {
      runsById,
      ...
      collection: {
        ...
        stale:
          event.status !== "ready" &&
          (state.collection.stale ||
            state.collection.status === "ready" ||
  ...
  const nextRun = applyToRun(state.runsById[runId], runMessage);
...
export interface AgentToolBroadcastHooks {
   ...
   * on one monotonic line. It must never be realigned to a stored index: an
   * atomic inspection uses it as the exclusive live replay boundary.
   */
  liveSequences: Map<string, number>;
  /** Opaque live-producer epoch, shared with atomic replay snapshots. */
  epoch?: string;
