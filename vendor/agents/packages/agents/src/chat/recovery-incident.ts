...
export async function setChatRecovering(
  ...
  if (active) {
    if (
      activeExisting &&
      (requestId === undefined || existing.requestId === requestId)
    )
      return; // same recovery owner — idempotent, no timestamp refresh
    ...
    // Old fibers and scheduled incident cleanup may finish after a newer
    // request starts recovering. Only an unscoped Stop clears any owner.
    // Records without an identity retain the legacy terminal-cleanup behavior.
    if (
      requestId !== undefined &&
