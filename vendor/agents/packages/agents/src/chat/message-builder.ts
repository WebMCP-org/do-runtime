...
function toolStateRank(part: MessagePart): number {
  ...
  if (part.state === "approval-responded") return 3;
  if (part.state === "approval-requested") return 2;
  if (part.state === "input-available") return 1;
  return 0;
}

/** Keep one part per toolCallId, preferring the furthest-progressed state. */
export function dedupePartsByToolCallId(parts: MessageParts): MessageParts {
  const indexes = new Map<string, number>();
  const deduped: MessageParts = [];
  ...
  for (const part of parts) {
    ...
    const existingIndex = indexes.get(part.toolCallId);
    if (existingIndex === undefined) {
      indexes.set(part.toolCallId, deduped.length);
      deduped.push(part);
      continue;
    }

    const existing = deduped[existingIndex];
    if (toolStateRank(part) >= toolStateRank(existing)) {
