...
class FakeStorage {
  ...
  async delete(keys: string | string[]): Promise<boolean | number> {
    if (typeof keys === "string") return this.data.delete(keys);
...
describe("setChatRecovering ownership", () => {
  function setup() {
    const storage = new FakeStorage();
    const frames: Record<string, unknown>[] = [];
    const set = (active: boolean, requestId?: string, now = T0) =>
      setChatRecovering(active, requestId, {
        storage: storage as unknown as Pick<
          DurableObjectStorage,
          "get" | "put" | "delete"
        >,
        messageType: "recovering",
        broadcast: (frame) => frames.push(frame),
        now
      });
    return { storage, frames, set };
  }

  it("records a newer owner and ignores the older request's late cleanup", async () => {
    const { storage, frames, set } = setup();
    await set(true, "old-request");
    await set(true, "new-request", T0 + 1);
    await set(false, "old-request", T0 + 2);

    expect(storage.data.get(CHAT_RECOVERING_KEY)).toEqual({
      requestId: "new-request",
      at: T0 + 1
    });
    expect(frames).toEqual([
      { type: "recovering", recovering: true, id: "old-request" },
      { type: "recovering", recovering: true, id: "new-request" }
    ]);

    await set(false, "new-request", T0 + 3);
    expect(storage.data.has(CHAT_RECOVERING_KEY)).toBe(false);
  ...
  it("keeps a repeated owner idempotent without rejuvenating its timestamp", async () => {
    const { storage, frames, set } = setup();
    await set(true, "request");
    await set(true, "request", T0 + 1);
    await set(true, undefined, T0 + 2);
    ...
    await set(true, "request", T0 + CHAT_RECOVERING_FLAG_TTL_MS);
    expect(storage.data.get(CHAT_RECOVERING_KEY)).toEqual({
      requestId: "request",
      at: T0 + CHAT_RECOVERING_FLAG_TTL_MS
    });
    expect(frames).toHaveLength(2);
  });

  it("allows an explicit unscoped Stop to clear the current owner", async () => {
    const { storage, frames, set } = setup();
    await set(true, "request");
    await set(false);
  ...
  it("preserves terminal cleanup of a legacy record with no request identity", async () => {
    const { storage, frames, set } = setup();
    storage.data.set(CHAT_RECOVERING_KEY, { at: T0 });
    await set(false, "request");

    expect(storage.data.has(CHAT_RECOVERING_KEY)).toBe(false);
    expect(frames).toEqual([
      { type: "recovering", recovering: false, id: "request" }
    ]);
  });
});
