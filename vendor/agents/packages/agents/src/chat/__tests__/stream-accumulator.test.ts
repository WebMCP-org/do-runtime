...
function settledToolPart(): UIMessage["parts"][number] {
  return {
    type: "tool-lookup",
    toolCallId: "tool-1",
...
describe("StreamAccumulator", () => {
  ...
  describe("mergeInto", () => {
    ...
    it("restarts a lazily seeded continuation without losing history or duplicating replay", () => {
      const a = acc({ messageId: "continued", continuation: true });
      const append = (target: StreamAccumulator) => {
        target.applyChunk({ type: "text-start" } as StreamChunkData);
        target.applyChunk({
          type: "text-delta",
          delta: "continued"
        } as StreamChunkData);
      ...
      append(a);
      const live = a.mergeInto([userMsg, assistantMsg]);
      const replay = a.restartFromSeed();
      append(replay);
      expect(replay.mergeInto(live)).toEqual(live);
      expect(assistantMsg.parts).toEqual([{ type: "text", text: "hi" }]);
      expect(live[1].parts).toMatchObject([
        { text: "hi" },
        { text: "continued" }
      ]);
    });

    it("continuation with a matching message id leaves sibling messages untouched", () => {
      const toolPart = settledToolPart();
      const siblingParts: UIMessage["parts"] = [toolPart];
      ...
      const a = acc({
        continuation: true,
        existingParts: [toolPart]
      });

      const result = a.mergeInto([userMsg, sibling, current]);

      expect(result[1]).toBe(sibling);
      expect(result[1].parts).toBe(siblingParts);
      expect(
        result
          .flatMap((message) => message.parts)
          .filter(
            (part) => "toolCallId" in part && part.toolCallId === "tool-1"
          )
      ).toHaveLength(2);
    });

    it("continuation removes a superseded assistant snapshot with the same tool call", () => {
      const toolPart = settledToolPart();
      ...
      const result = a.mergeInto([userMsg, stale, latest]);
      ...
      expect(result.some((message) => message.id === "stale")).toBe(false);
      expect(a.messageId).toBe("latest");

      const laterSibling: UIMessage = {
        id: "later-sibling",
        role: "assistant",
        parts: [toolPart]
      };
      const next = a.mergeInto([
        userMsg,
        laterSibling,
        result[result.length - 1]
      ]);

      expect(next[1]).toBe(laterSibling);
    });

    it("continuation preserves sibling text while removing its duplicate tool call", () => {
      const toolPart = settledToolPart();
      const stale: UIMessage = {
        id: "stale",
        role: "assistant",
        parts: [{ type: "text", text: "keep me" }, toolPart]
      };
      const latest: UIMessage = {
        id: "latest",
        role: "assistant",
      ...
      expect(result.find((message) => message.id === "stale")?.parts).toEqual([
    ...
    it("returns a new array (does not mutate input)", () => {
      ...
      a.applyChunk({ type: "text-start" } as StreamChunkData);
      expect(result.at(-1)?.parts).toHaveLength(1);
