...
describe("agent tool event reducer", () => {
  it("retains rows while loading or failed and clears every index for a ready empty roster", () => {
    let state = applyAgentToolEvent(
      createAgentToolEventState(),
      frame(0, {
        kind: "started",
        runId: "bound",
    ...
    for (const event of [
      ...
      expect(state.runsById).toEqual(retained.runsById);
      expect(state.runsByToolCallId).toEqual(retained.runsByToolCallId);
      expect(state.unboundRuns).toEqual(retained.unboundRuns);
    ...
    state = applyAgentToolEvent(state, {
      ...
      replayId: "reconnect",
      sequence: 1,
      event: { kind: "collection", status: "ready", runIds: [] }
    });
    expect(state).toEqual({
      runsById: {},
      runsByToolCallId: {},
      unboundRuns: [],
      collection: { status: "ready", stale: false }
    });
  });

  it("retains a legacy milestone from replay chunks when inspection omits milestones", () => {
    const milestone = { name: "saved", sequence: 0, at: 123 };
    ...
    let state = applyAgentToolEvent(
      ...
      frame(0, {
        ...
        snapshot: { replay: { chunks: [{ sequence: 0, body }] } }
      })
    );
    state = applyAgentToolEvent(
      state,
      frame(1, { kind: "chunk", runId: "r", body })
    );
    expect(state.runsById.r.milestones).toEqual([milestone]);
    expect(
      state.runsById.r.parts.filter(
        (part) => part.type === AGENT_TOOL_MILESTONE_PART
      )
    ).toHaveLength(1);
  });
