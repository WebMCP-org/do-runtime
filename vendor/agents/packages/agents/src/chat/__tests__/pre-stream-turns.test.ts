...
describe("PreStreamTurns (#1784)", () => {
  ...
  it("ordinary park is advisory until the request is persisted", () => {
  ...
  it("marks a request persisted and acknowledges the origin plus duplicate waiters", () => {
    const originFrames: SentFrame[] = [];
    const duplicateFrames: SentFrame[] = [];
    const pre = new PreStreamTurns();
    const origin = makeConnection("origin", originFrames);
    const duplicate = makeConnection("duplicate", duplicateFrames);

    pre.begin("req-1");
    expect(pre.parkDuplicate(duplicate, "req-1")).toBe(true);
    expect(duplicateFrames).toEqual([
      { type: "cf_agent_stream_pending", id: "req-1" }
    ]);

    pre.markPersisted("req-1", origin);

    expect(originFrames).toEqual([
      { type: "cf_agent_stream_pending", id: "req-1", persisted: true }
    ]);
    expect(duplicateFrames.at(-1)).toEqual({
      type: "cf_agent_stream_pending",
      id: "req-1",
      persisted: true
    });
    expect(pre.isPersisted("req-1")).toBe(true);

    pre.settle("req-1");
    expect(pre.isPersisted("req-1")).toBe(false);
  });
