...
describe("broadcast stream state machine", () => {
  ...
  it("replayed continuation restarts from its original seed", () => {
    let current = makeMessages("hi", "first half");
    const chunks = [
    ...
    const firstReplay = transition(state, {
      type: "response",
      streamId: "req-c",
      messageId: "tmp",
      replay: true,
    ...
    current = firstReplay.messagesUpdate!(current);
    state = firstReplay.state;
    ...
    expect(
      assistant?.parts.some(
        (part) => part.type === "text" && part.text === "first half"
      )
    ).toBe(true);
  });
