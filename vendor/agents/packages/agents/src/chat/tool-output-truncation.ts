...
function truncateString(
  ...
  // Keep a tail as well as a head: the end of a long output is where an error
  // message lands, and a head-only cut buries exactly that. `keptChars` splits
  // 4:1 head to tail once the marker is reserved, so the result still lands
  // exactly on `maxChars`; too small a budget keeps the head-only shape.
  const marker = `${suffix} ... `;
  const keptChars = maxChars - marker.length;
  const tailChars = Math.floor(keptChars / 5);
  if (tailChars < 1) {
    return `${value.slice(0, maxChars - suffix.length)}${suffix}`;
  }

  const head = value.slice(0, keptChars - tailChars);
