...
export class StreamAccumulator {
  ...
  private readonly seedParts: UIMessage["parts"];
  private seedMetadata?: Record<string, unknown>;
  metadata?: Record<string, unknown>;
  ...
  constructor(options: StreamAccumulatorOptions) {
    ...
    this.seedParts = options.existingParts
      ? options.existingParts.map((part) => ({ ...part }))
      : [];
    this.parts = this.seedParts.map((part) => ({ ...part }));
    this.seedMetadata = options.existingMetadata
  ...
  mergeInto(messages: UIMessage[]): UIMessage[] {
    ...
    let usedContinuationFallback = false;

    if (existingIdx < 0 && this._isContinuation) {
      for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i].role === "assistant") {
          existingIdx = i;
          this.messageId = messages[i].id;
    ...
    if (this._pendingContinuationChunks !== null) {
      ...
      this.seedMetadata = currentMetadata ? { ...currentMetadata } : undefined;
      if (existingIdx >= 0) {
    ...
    const partialMessage: UIMessage = {
      id: messageId,
      role: "assistant",
      parts: dedupePartsByToolCallId(this.parts),
      ...(this.metadata != null && { metadata: this.metadata })
    } as UIMessage;
    ...
    if (existingIdx >= 0) {
      ...
      if (!usedContinuationFallback) return updated;

      const ownedToolCalls = new Set(
        partialMessage.parts.filter(isToolUIPart).map((part) => part.toolCallId)
      );
      if (ownedToolCalls.size === 0) return updated;
      return updated.flatMap((message, index) => {
        if (index === existingIdx || message.role !== "assistant") {
          return [message];
        }
        const parts = message.parts.filter(
          (part) => !isToolUIPart(part) || !ownedToolCalls.has(part.toolCallId)
        );
