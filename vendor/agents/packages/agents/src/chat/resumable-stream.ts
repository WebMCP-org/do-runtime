...
export class ResumableStream {
  ...
  hasRequest(requestId: string): boolean {
    ...
    const rows = this.sql<{ found: number }>`
      select 1 as found from cf_ai_chat_stream_metadata
  ...
  hasErroredRequest(requestId: string): boolean {
    return this._latestStreamForRequest(requestId, "error") !== undefined;
  }

  /**
   * Retain a completed request that terminalized before creating a stream.
   * This uses the existing stream-metadata owner so duplicate admission and
   * replay follow the same retention and clear semantics as streamed turns.
   */
  recordCompletedRequest(requestId: string): void {
    ...
    this.sql`
      insert into cf_ai_chat_stream_metadata
        (id, request_id, status, created_at, completed_at, is_continuation)
      values (${nanoid()}, ${requestId}, 'completed', ${now}, ${now}, 0)
    `;
  }
