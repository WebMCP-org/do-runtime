...
export class ResumableStream {
  ...
  /**
   * Whether retained stream metadata already owns this request id. Used by the
   * chat admission path to route a replay through the existing resume protocol
   * instead of executing the same request twice.
   */
  hasRequest(requestId: string): boolean {
    this.flushBuffer();
    return this._latestChatRowByTag(requestId) !== undefined;
  }

  /** Whether this request retains an errored stream that can be replayed. */
  hasErroredRequest(requestId: string): boolean {
    return this._latestChatRowByTag(requestId, "errored") !== undefined;
  }

  /**
   * Retain a completed request that terminalized before creating a stream.
   * This uses the existing stream owner so duplicate admission and replay
   * follow the same retention and clear semantics as streamed turns: a
   * chunkless chat-owned row settled `completed`, reclaimed with every other
   * finished chat stream.
   */
  recordCompletedRequest(requestId: string): void {
    if (this.hasRequest(requestId)) return;
    const streamId = nanoid();
    this.ops.insertStream(streamId, requestId, { cfChat: 1 });
    this.ops.settle(streamId, "completed", null);
  }
