...
>(
  ...
  function doGetInitialMessages(
    ...
    // A rejected hydration must not stay cached. `use()` throws the rejection
    // during render, so the component never commits and the effect below that
    // would evict this entry never runs — leaving every later attempt, error
    // boundary retries included, to rethrow the same settled rejection instead
    // of reaching the agent again.
    void promise.catch(() => {
  ...
  const resetLocalChatState = useCallback(() => {
    const chatId = stableChatIdRef.current;
    if (chatId) customTransport.discardPendingRequest(chatId);
    markInitialMessagesSeeded();
    setMessages([]);
  ...
  useEffect(() => {
    ...
    function onAgentMessage(event: MessageEvent) {
      ...
      switch (data.type) {
        ...
        case MessageType.CF_AGENT_STREAM_PENDING:
          ...
          customTransport.handleStreamPending(data);
          break;
        ...
        case MessageType.CF_AGENT_USE_CHAT_RESPONSE: {
          ...
          if (result.isStreaming) {
            // Observing a broadcast turn renders a stop button (isStreaming
            // drives the composer), but stop() only cancels what the transport
            // knows about: AI SDK status is "ready" here, so Chat.stop() is a
            // no-op and cancelActiveServerTurn() sends nothing without a
            // registered request id. Register it, like the resume-fallback
    ...
    const releaseAgentChatReplaySubscription = _drainAgentChatReplayFrames(
      agent,
      (data) => {
        onAgentMessage(new MessageEvent("message", { data }));
      }
    );
    ...
    return () => {
      ...
      releaseAgentChatReplaySubscription();
      fallbackAckedResumeRequestIds.clear();
      streamStateRef.current = { status: "idle" };
      setIsServerStreaming(false);
      setIsRecovering(false);
