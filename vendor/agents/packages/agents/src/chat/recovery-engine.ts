...
export interface ChatFiberWakeHooks<TClassify> {
  ...
  /** Explicit cancellation is durable even when the original fiber survived. */
  isRecoveryCancelled?(snapshot: ChatFiberSnapshot | null): boolean;
  /**
...
export class ChatRecoveryEngine {
  ...
  async handleChatFiberRecovery<TClassify>(
    ...
    if (wake.isRecoveryCancelled?.(snapshot)) {
      if (
      ...
      await adapter.setRecovering(
        false,
        snapshot?.recoveryRootRequestId ?? requestId
      );
      return true;
    }
