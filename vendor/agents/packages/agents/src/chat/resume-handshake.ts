...
export class ResumeHandshake {
  ...
  /**
   * Claim a fresh request id or route a replay to its existing owner. After the
   * terminal-storage read, the owner check and fresh claim run in one synchronous
   * continuation. That claim is visible before the caller resumes, so two
   * concurrent deliveries cannot both pass admission. Returns `true` only for
   * the caller that owns the fresh claim; duplicate callers have already been
   * routed and must stop.
   */
  async claimRequest(
    connection: Connection,
    requestId: string
  ): Promise<boolean> {
    if (this._routeOwnedRequest(connection, requestId)) return false;

    const terminal = await this.host.pendingChatTerminal();
    if (this._routeOwnedRequest(connection, requestId)) return false;
    if (terminal?.requestId === requestId) {
      this._notifyRequestResuming(connection, requestId);
      return false;
    }

    this.host.preStream.begin(requestId);
    return true;
  }

  private _routeOwnedRequest(
    connection: Connection,
    requestId: string
  ): boolean {
    const { resumableStream, preStream } = this.host;
    if (resumableStream.hasRequest(requestId)) {
      this._notifyRequestResuming(connection, requestId);
      return true;
    }
    return preStream.parkDuplicate(connection, requestId);
  }
  ...
  notifyStreamResuming(connection: Connection, probeId?: string): void {
    const { resumableStream } = this.host;
    const requestId = resumableStream.activeRequestId;
    if (!resumableStream.hasActiveStream() || requestId === null) return;
    this._notifyRequestResuming(connection, requestId, probeId);
  }

  private _notifyRequestResuming(
    connection: Connection,
    requestId: string,
    probeId?: string
  ): void {
    const { pendingResumeConnections } = this.host;
  ...
  async handleResumeAck(
    ...
    if (
      ...
      // A completed request can be replayed while a different request is active.
    } else if (resumableStream.hasErroredRequest(requestId)) {
      if (
        resumableStream.replayErroredChunksByRequestId(connection, requestId)
      ) {
        sendIfOpen(
          connection,
          JSON.stringify({
