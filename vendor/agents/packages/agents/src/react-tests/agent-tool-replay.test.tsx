 ...
 *
 *   - Every connection receives a complete retained roster before child reads.
 *     Replay identities isolate each hydration attempt from older responses.
 ...
 *     real `reconnect()` without doubling its existing transcript.
...
function Harness({
  ...
  const projection = useAgentToolEvents({
    agent
  });
  ...
  return (
    <div>
      ...
      <div data-testid="tool-state">{JSON.stringify(projection)}</div>
    </div>
  );
}
...
describe("agent-tool replay over a real Worker", () => {
  it("keeps retained metadata when the child is unavailable without recreating it", async () => {
    const name = `replay-missing-child-${crypto.randomUUID()}`;
    const driver = await mount(name);
    const runId = crypto.randomUUID();
    await driver.getAgent().call("runDeterministicAgentToolForTest", [
      {
        runId,
        chunkBodies: CHUNK_BODIES,
        summary: "retained result"
      }
    ]);
    await driver.getAgent().call("deleteRetainedChildForTest", [runId]);
    const observer = await mount(name);
    await vi.waitFor(() => {
      expect(readState(observer.container).runsById[runId]).toMatchObject({
        status: "completed",
        summary: "retained result"
      });
    ...
    await expect(
      driver.getAgent().call("hasRetainedChildForTest", [runId])
    ).resolves.toBe(false);
  });

  it("distinguishes a complete empty roster from a collection still loading", async () => {
    const observer = await mount(`replay-empty-${crypto.randomUUID()}`);
    await vi.waitFor(() => {
      expect(readState(observer.container).collection).toEqual({
        status: "ready",
        stale: false
      });
      expect(readState(observer.container).runsById).toEqual({});
    });
  });

  it("hydrates persisted progress and milestones without another child update", async () => {
    const name = `replay-progress-${crypto.randomUUID()}`;
    const driver = await mount(name);
    const progress = { message: "Sources checked", fraction: 0.75, at: 1234 };
    const milestones = [
      {
        name: "report-1",
        sequence: 0,
        at: 1000,
        data: { message: "Found sources" }
      },
      {
        name: "report-2",
        sequence: 1,
        at: 1234,
        data: { message: "Checked sources" }
      }
    ];
    await driver.getAgent().call("runDeterministicAgentToolForTest", [
      {
    ...
    const observer = await mount(name);
    await vi.waitFor(() => {
      const run = readState(observer.container).runsById["progress-run"];
      expect(run?.progress).toEqual(progress);
      expect(run?.milestones).toEqual(milestones);
