...
describe("SubAgent", () => {
  ...
  describe("broadcast paths on facets", () => {
    it.each(["connect", "message"] as const)(
      "retains a facet connection after its %s handler has returned",
      async (handler) => {
        const parentName = uniqueName();
        const childName = uniqueName();
        const parent = await getAgentByName(env.TestSubAgentParent, parentName);
        ...
        try {
          ...
          if (handler === "message") {
            const refreshed = waitForJsonMessage<{ type: string }>(
              ws,
              (data) => data.type === "connection-refreshed"
            );
            ws.send("refresh-connection");
            await refreshed;
          }

          // A separate parent -> child RPC cannot borrow the already-returned
          // connect/message handler's parameter. Repeated lookups must keep
          // the same owned bridge usable.
          for (const message of ["first later RPC", "second later RPC"]) {
            const delivered = waitForJsonMessage<{
              type: string;
              message: string;
            }>(ws, (data) => data.type === "retained-connection");
            await parent.subAgentSendToRetainedConnection(childName, message);
            expect(await delivered).toEqual({
              type: "retained-connection",
              message
            });
          }
        } finally {
          ws.close();
          await expect
            .poll(() => parent.subAgentConnectionCount(childName))
            .toBe(0);
        }
      }
    );
