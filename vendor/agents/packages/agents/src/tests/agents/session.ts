...
export class TestSessionAgent extends Agent {
  ...
  /**
   * Vendor divergence coverage: the request-level retention `ResumableStream`
   * exposes to chat admission, so a re-delivered request replays instead of
   * running inference twice. A turn that settled without ever streaming keeps
   * an empty completed receipt in the same stream owner as streamed turns.
   */
  async resumableRequestRetentionForTest(): Promise<{
    unknownRequest: boolean;
    streamedRequest: boolean;
    erroredRequest: boolean;
    recordedRequest: boolean;
    recordedRequestRows: number;
    recordedRequestIsErrored: boolean;
  }> {
    const stream = new ResumableStream(
      this.streams,
      <T = Record<string, unknown>>(
    ...
    const unknownRequest = stream.hasRequest("req-unknown");

    const streamId = stream.start("req-streamed");
    stream.storeChunk(streamId, '{"type":"text-delta","delta":"a"}');
    stream.markError(streamId);
    const streamedRequest = stream.hasRequest("req-streamed");
    const erroredRequest = stream.hasErroredRequest("req-streamed");

    // A receipt is recorded once; a repeat call must not add a second row.
    stream.recordCompletedRequest("req-empty");
    stream.recordCompletedRequest("req-empty");
    const recordedRequest = stream.hasRequest("req-empty");
    const recordedRequestRows = this.sql<{ count: number }>`
      SELECT COUNT(*) AS count FROM cf_agents_streams WHERE tag = 'req-empty'
    `[0].count;
    ...
    return {
      ...
      recordedRequest,
      recordedRequestRows,
      recordedRequestIsErrored: stream.hasErroredRequest("req-empty")
    };
  }
}
