...
export class BroadcastSubAgent extends Agent<Cloudflare.Env, BroadcastState> {
  ...
  override onMessage(
    ...
    if (message !== "refresh-connection") return;
    connection.send(JSON.stringify({ type: "connection-refreshed" }));
  }

  /** Send from a later RPC, after the socket handler's bridge argument expired. */
  sendToRetainedConnection(message: string): void {
    const [connection] = this.getConnections();
    if (!connection || this.getConnection(connection.id) !== connection) {
      throw new Error("Expected the facet's retained connection");
    }
    connection.send(JSON.stringify({ type: "retained-connection", message }));
  }
...
export class TestSubAgentParent extends Agent {
  ...
  async subAgentSendToRetainedConnection(
    subAgentName: string,
    message: string
  ): Promise<void> {
    const child = await this.subAgent(BroadcastSubAgent, subAgentName);
    await child.sendToRetainedConnection(message);
  }
