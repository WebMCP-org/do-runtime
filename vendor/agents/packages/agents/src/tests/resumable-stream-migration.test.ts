...
describe("ResumableStream — legacy table migration", () => {
  ...
  it("retains request receipts for duplicate chat admission", async () => {
    const agent = await getAgentByName(
      env.TestSessionAgent,
      crypto.randomUUID()
    );
    const result = await (
      agent as unknown as {
        resumableRequestRetentionForTest(): Promise<{
    ...
    ).resumableRequestRetentionForTest();

    expect(result.unknownRequest).toBe(false);
    // A streamed turn is owned by its stream row, errored ones replayably so.
    expect(result.streamedRequest).toBe(true);
    expect(result.erroredRequest).toBe(true);
    // A turn that settled without streaming keeps an empty completed receipt.
    expect(result.recordedRequest).toBe(true);
    expect(result.recordedRequestRows).toBe(1);
    expect(result.recordedRequestIsErrored).toBe(false);
  });
});
