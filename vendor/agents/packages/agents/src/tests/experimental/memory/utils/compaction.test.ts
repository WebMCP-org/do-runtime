...
describe("truncateOlderMessages", () => {
  ...
  it("keeps the tail of a truncated string output", () => {
    const tail = "Error: the part that matters";
    const content = `${"a".repeat(900)}${tail}`;
    const messages = [
      toolMessage("old-tool", content),
      textMessage("recent-1", "recent one"),
      textMessage("recent-2", "recent two")
    ];
    ...
    const output = firstOutput(truncated[0]) as string;
  ...
  it("keeps the head-only shape when the budget cannot fit a tail", () => {
    ...
    const truncated = truncateOlderMessages(messages, {
      keepRecent: 2,
      // Clears the `[truncated N chars]` marker but leaves no room for a tail.
      maxToolOutputChars: 30
    });

    expect(firstOutput(truncated[0])).toBe(
      `zzzz... [truncated ${content.length} chars]`
    );
  });
