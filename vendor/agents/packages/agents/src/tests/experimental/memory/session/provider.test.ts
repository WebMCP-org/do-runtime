...
import type { SessionMessage } from "../../../../experimental/memory/session";
...
interface SessionAgentStub {
  ...
  getHistory(leafId?: string): Promise<SessionMessage[]>;
...
describe("AgentSessionProvider — tree-structured messages", () => {
  ...
  it("restores the stored creation time on every message read", async () => {
    const agent = await getAgent(name);
    const explicitCreatedAt = new Date("2026-08-12T12:34:56.789Z");
    await agent.appendMessage({
      id: "m1",
      role: "user",
      parts: [{ type: "text", text: "Hello" }],
    ...
    await agent.appendMessage({
      id: "m3",
      role: "user",
      parts: [{ type: "text", text: "Invalid embedded date" }],
      createdAt: "not-a-date"
    } as unknown as SessionMessage);

    const message = await agent.getMessage("m1");
    const invalidEmbeddedTime = await agent.getMessage("m3");
    const history = await agent.getHistory();
    const recent = (await agent.getRecentHistory(1024 * 1024)).messages;
    const branches = await agent.getBranches("m1");

    expect(message?.createdAt).toEqual(explicitCreatedAt);
    expect(invalidEmbeddedTime?.createdAt).toBeInstanceOf(Date);
    expect(history.every((item) => item.createdAt instanceof Date)).toBe(true);
    expect(recent.every((item) => item.createdAt instanceof Date)).toBe(true);
  ...
  it("updateMessage", async () => {
    const agent = await getAgent(name);
    const createdAt = new Date("2026-08-12T12:34:56.789Z");
    await agent.appendMessage({
      id: "m1",
      role: "user",
  ...
  it("updateMessage repairs a corrupt stored message", async () => {
    const agent = await getAgent(name);
    await agent.appendMessage({
      id: "m1",
      role: "user",
    ...
    await agent.corruptMessageForTest("m1");
    ...
    const message = await agent.getMessage("m1");
    expect(message?.parts[0]).toEqual({ type: "text", text: "Repaired" });
  ...
  it("compaction overlays — addCompaction replaces range in getHistory", async () => {
    ...
    const history = await agent.getHistory();
    const repeatedHistory = await agent.getHistory();
    ...
    const [compaction] = (await agent.getCompactions()) as Array<{
      createdAt: number;
    }>;
    expect(history[1].createdAt).toEqual(new Date(compaction.createdAt));
    expect(repeatedHistory[1].createdAt).toEqual(history[1].createdAt);
...
describe("AgentSessionProvider — chunked history hydration (#1710)", () => {
  ...
  it("long history spanning several chunks preserves root→leaf order", async () => {
    ...
    expect(history.every((message) => message.createdAt instanceof Date)).toBe(
      true
    );
