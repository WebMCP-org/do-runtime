...
export class AgentSessionProvider implements SessionProvider {
  ...
  getMessage(id: string): SessionMessage | null {
    this.ensureTable();
    const row = this.agent.sql<StoredMessageRow>`
      SELECT content, strftime('%Y-%m-%dT%H:%M:%fZ', created_at) AS createdAt
      FROM assistant_messages WHERE id = ${id} AND session_id = ${this.sessionId}
    `[0];
    return row ? this.parse(row.content, row.createdAt) : null;
  }
  ...
  getBranches(messageId: string): SessionMessage[] {
    this.ensureTable();
    const rows = this.agent.sql<StoredMessageRow>`
      SELECT content, strftime('%Y-%m-%dT%H:%M:%fZ', created_at) AS createdAt
  ...
  updateMessage(message: SessionMessage): void {
    this.ensureTable();
    const stored = this.agent.sql<StoredMessageRow>`
      SELECT content, strftime('%Y-%m-%dT%H:%M:%fZ', created_at) AS createdAt
      FROM assistant_messages WHERE id = ${message.id} AND session_id = ${this.sessionId}
    `[0];
    if (!stored) return;
    const createdAt =
      this.parse(stored.content, stored.createdAt)?.createdAt ??
      parseDate(stored.createdAt);
    const updated = createdAt ? { ...message, createdAt } : message;
    this.agent.sql`
      UPDATE assistant_messages
      SET role = ${updated.role}, content = ${JSON.stringify(updated)}
  ...
  getCompactions(): StoredCompaction[] {
    ...
    return this.agent.sql<Row>`
      SELECT id, summary, from_message_id, to_message_id,
        strftime('%Y-%m-%dT%H:%M:%fZ', created_at) AS createdAt
      FROM assistant_compactions
      WHERE session_id = ${this.sessionId} ORDER BY created_at ASC, rowid ASC
    `.map((r) => ({
      id: r.id,
      summary: r.summary,
  ...
  private messagesByPathStats(rows: HistoryRowStat[]): SessionMessage[] {
    const contentById = new Map<string, StoredMessageRow>();
    const fetchChunk = (ids: string[]) => {
      const fetched = this.agent.sql<StoredMessageRow & { id: string }>`
        SELECT id, content, strftime('%Y-%m-%dT%H:%M:%fZ', created_at) AS createdAt
        FROM assistant_messages
    ...
    for (const row of rows) {
      ...
      const msg = this.parse(stored.content, stored.createdAt);
  ...
  private parse(json: string, storedCreatedAt: string): SessionMessage | null {
    try {
      ...
      if (
        ...
        const createdAt =
          parseDate(msg.createdAt) ?? parseDate(storedCreatedAt);
        return createdAt ? { ...msg, createdAt } : msg;
      }
    } catch {
      /* skip */
    }
  ...
  private parseRows(rows: StoredMessageRow[]): SessionMessage[] {
    const result: SessionMessage[] = [];
    for (const row of rows) {
      const msg = this.parse(row.content, row.createdAt);
      if (msg) result.push(msg);
    }
    return result;
  }
