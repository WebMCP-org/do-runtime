...
export interface StateOptions<T = unknown> {
  ...
  /**
   * Vendor divergence: lossless hydration. When set, {@link State.get}
   * decodes a stored row without the corrupt-row fallback and passes it
   * through this hook. A decode or migration throw leaves the row untouched
   * and caches nothing, so a later read retries. A changed value (not the
   * input by reference) is written back without validation or `onChanged`.
   */
  readonly migratePersistedState?: (state: unknown) => T;
}
...
export class State<T = unknown> extends LifecycleCapability {
   ...
   * the initial state (re-persisting it) or clears the row, unless
   * `migratePersistedState` is set (vendor divergence: then it throws).
   */
  get(): T | undefined {
    ...
    if (result.length > 0) {
      // Vendor divergence: lossless hydration (see StateOptions). Decode
      // before caching or writing; a malformed row belongs to the application.
      const migrate = this.#options.migratePersistedState;
      if (migrate) {
        const parsed: unknown = JSON.parse(result[0].state ?? "null");
