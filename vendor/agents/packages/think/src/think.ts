...
// Vendor divergence: `SkillScriptRunner` is not imported. The fork removed
// `getSkillScriptRunner()` and upstream's Codemode-backed `run_skill_script`
// injection (2026-08-16 "Active Think and skill barrels no longer reach
// Codemode"), so the type has no referent here.
import type { SkillSource, SkillWorkspaceSeedOptions } from "agents/skills";
...
// Vendor divergence: stamp the turn start plus each completed step's usage
// onto message metadata. The last step wins, so usage remains current context
// occupancy rather than a turn-wide sum.
function turnMessageMetadata(startedAt: number) {
  const createdAt = new Date(startedAt).toISOString();
  return ({ part }: { part: { type: string } }): unknown =>
    part.type === "start"
      ? { createdAt }
      : part.type === "finish-step" && "usage" in part
        ? { usage: part.usage }
        : undefined;
}

type SessionUIMessage = UIMessage & Pick<SessionMessage, "createdAt">;

function messageCreatedAt(message: UIMessage): Date | undefined {
  const stored = (message as SessionUIMessage).createdAt;
  if (stored instanceof Date && !Number.isNaN(stored.getTime())) return stored;

  const projected = (message.metadata as ThinkMessageMetadata | undefined)
    ?.createdAt;
...
function withMessageCreatedAt(
  message: UIMessage,
  createdAt: Date
): SessionUIMessage {
  const metadata = message.metadata as Record<string, unknown> | undefined;
  const topLevelCreatedAt = (message as SessionUIMessage).createdAt;
  const iso = createdAt.toISOString();
  if (
    metadata?.createdAt === iso &&
    topLevelCreatedAt instanceof Date &&
    topLevelCreatedAt.getTime() === createdAt.getTime()
  ) {
    return message as SessionUIMessage;
  }
  return {
    ...message,
    createdAt,
...
export type ThinkTerminalMessageStatus = "completed" | "error" | "aborted";

function withTerminalMessageTiming(
  message: UIMessage,
  status: ThinkTerminalMessageStatus,
  startedAt: number
): UIMessage {
  return {
    ...message,
...
export function tryParseDeclaredTaskSchedule(
  rawSchedule: string,
  taskTimezone: string | undefined,
  defaultTimezone: string | undefined
...
/** A parked `kind: "durable-pause"` action awaiting a human decision. */
...
const admittedTurnContext = new AsyncLocalStorage<{
  ...
  stopEpoch: number;
  recoveryRootRequestId?: string | undefined;
...
export type ParsedDeclaredSchedule =
  | {
      kind: "interval";
      intervalMs: number;
...
export type ParseDeclaredScheduleResult =
 ...
 */
const RESERVED_MESSAGE_METADATA_KEYS = [
  "channel",
  "turnMetadata",
  "createdAt",
  "durationMs",
...
/**
 * Vendor divergence: a key-order-insensitive JSON projection, used only to
 * decide whether an echoed client message already matches its stored row.
 *
 * Think re-stamps the durable creation time on every message it writes
 * (2026-08-12 "Session and turn timing reach chat metadata"), and that moves
 * `createdAt` to the end of a client-echoed object while the stored row still
 * carries it where it was first written. A literal `JSON.stringify` compare
 * would therefore never match, and the whole echoed transcript would be
 * re-written on every turn. `JSON.stringify` applies `toJSON` before the
 * replacer, so a `Date` and the ISO string it round-trips to compare equal
 * here too.
 */
...
// Vendor divergence: upstream's module-level `stripReservedMetadata()` helper
// is not carried. It existed only to predict what Sessions' `source: "client"`
// write would store, and this fork does not route intake through that mode —
// `_stripReservedMessageFields` strips the reserved keys itself and
// `_persistIncomingMessage` then restores the server's own values, which the
// Sessions-side strip would erase (2026-08-23 / 2026-08-12).
...
/** Durable chat timing metadata emitted by Think on UI messages. */
export type ThinkMessageMetadata = {
  createdAt?: string;
  durationMs?: number;
  status?: ThinkTerminalMessageStatus;
};
...
> extends Agent<Env, State, Props> {
  ...
  /**
   * Whether the configured model reads image parts. Enabled by default. Set to
   * `false`, or to a thunk for a host whose model can change between requests,
   * and a workspace `read` of an image renders as a note naming the file
   * instead of the picture.
   */
  workspaceAcceptsImages: NonNullable<
    Parameters<typeof createWorkspaceTools>[1]
  >["acceptsImages"] = true;

  /**
   * Whether the configured model reads PDF file parts. Enabled by default. Set
   * to `false`, or to a thunk, and a workspace `read` of a PDF renders as a
   * note naming the file instead of the document.
   */
  workspaceAcceptsPdfs: NonNullable<
    Parameters<typeof createWorkspaceTools>[1]
  >["acceptsPdfs"] = true;
  ...
  private _replaceCachedMessages(messages: UIMessage[]): UIMessage[] {
    this._cachedMessages = messages.map(Think._projectMessageCreatedAt);
    return this._cachedMessages;
  }
  ...
  /**
   * Vendor divergence: project the message's durable creation time into the
   * typed `createdAt` field (2026-08-12 "Session and turn timing reach chat
   * metadata"). Sessions 0.23 does not restore its SQL `created_at` onto
   * reads, so the `metadata.createdAt` stamped at write time is the only
   * authoritative source; this re-derives the typed field from it for the
   * live cache.
   */
  private static _projectMessageCreatedAt(message: UIMessage): UIMessage {
    const createdAt = messageCreatedAt(message);
    return createdAt ? withMessageCreatedAt(message, createdAt) : message;
  }
  ...
  private async _appendMessageToHistory(
    ...
    // Vendor divergence: stamp the turn's creation time before the write
    // (2026-08-12 "Session and turn timing reach chat metadata"). Sessions
    // 0.23 never restores its SQL `created_at` onto a read, so the
    // `metadata.createdAt` written here is the transcript's only record of
    // when the message was accepted.
    const result = await this.session.appendMessage(
      withMessageCreatedAt(message, messageCreatedAt(message) ?? createdAt),
      { parentId }
    );
  ...
  private async _updateMessageInHistory(
    ...
    // Vendor divergence: the stored row's creation time is authoritative — a
    // browser replay of an older snapshot must not move it (2026-08-12).
    // `existing` lets `_upsertMessageInHistory` reuse the read it already did.
    const stored =
      existing ??
      ((await this.session.getMessage(message.id)) as UIMessage | null);
    const stamped = withMessageCreatedAt(
      message,
      (stored && messageCreatedAt(stored)) ??
    ...
    return ((await this.session.updateMessage(stamped)) ??
      stamped) as UIMessage;
  }
  ...
  private async _upsertMessageInHistory(
    ...
    // Vendor divergence: upstream's `session.upsertMessage(..., { source })`
    // is not used here. Think stamps the message's durable creation time on
    // the write (2026-08-12) and an update must carry the STORED row's value
    // forward, which needs the existing row in hand; the read is reused by
    // `_updateMessageInHistory` so it still costs one lookup. Sessions'
    // `source: "client"` strip is likewise not used: `_persistIncomingMessage`
    // strips reserved metadata at intake itself and then restores the
    // server's own values, which a Sessions-side strip would erase.
    const existing = (await this.session.getMessage(
      message.id
    )) as UIMessage | null;
    if (existing) {
      return this._updateMessageInHistory(message, existing);
    }
  ...
  protected _orphanStore(): OrphanPersistStore {
    return {
      ...
      // Vendor divergence: orphan persistence routes through Think's history
      // helpers, not the raw Session handle, so a reconstructed orphan is
      // stamped with the same durable creation time as every other row
      // (2026-08-12).
      appendMessage: async (message, parentId) => {
        await this._appendMessageToHistory(message, parentId);
      },
  ...
  /**
   * Vendor divergence: bumped by every history-discarding reset (a chat
   * clear). A turn that was streaming when history was cleared compares the
   * generation it started under before persisting, so deleted history stays
   * deleted (2026-08-30 "Delegated work cancellation").
   */
  ...
  private readonly _agentToolLiveEpoch = crypto.randomUUID();
   ...
   * parent re-attaches and follows the child to its real terminal. Returning
   * the row's durable output contract also puts a structured recovery back
   * through the same workflow final-answer pipeline as its original turn.
   *
  ...
  private _rebindAgentToolChildRunRequestId(
    ...
    `;
    if (row.output_schema_json === null) return;
    return {
      workflow: {
        name: "agent-tool",
        id: row.run_id,
      ...
      output: { schema: JSON.parse(row.output_schema_json) as unknown }
    };
  }

  /** A child facet owns one run; a settled run must never admit recovery. */
  private _isStoppedRecovery(stopEpoch?: number): boolean {
    if (stopEpoch !== undefined && stopEpoch !== this._stopEpoch) return true;
    try {
      return (
        this.sql<{ run_id: string }>`
        SELECT run_id FROM cf_agent_tool_child_runs
        WHERE status NOT IN ('starting', 'running') LIMIT 1
      `.length > 0
      );
    } catch (error) {
  ...
  /** Persist a structured child answer before recovery reconciliation runs. */
  private _persistAgentToolOutputForRequest(
    requestId: string,
    output: unknown
  ): void {
  ...
  private get _stopEpoch(): number {
    return Number(this._configGet("turnStopEpoch") ?? 0);
  }
  ...
  /**
   * Run one messenger turn while its live delivery surface remains in the same
   * RPC call. This keeps the surface capability valid for `deliverNotice` and
   * avoids returning a restore callback across an Agent boundary.
   */
  async chatWithMessengerDelivery(
    userMessage: string | UIMessage,
    callback: StreamCallback,
    context: MessengerContext,
    surface: MessengerDeliverySurface
  ): Promise<void> {
    const restoreSurface = this.bindActiveDeliverySurface(surface);
    try {
      await this.chatWithMessengerContext(userMessage, callback, context);
    } finally {
      restoreSurface();
    }
  }
    ...
    try {
      if (channelId === "web") {
        ...
        const surface =
          ...
          (await this.internal_resolveMessengerDeliverySurface(
            channelId,
            options?.thread
          ));
        if (!surface) {
  ...
  async internal_reconcileChannels(): Promise<void> {
    ...
    let messengerRuntime: ThinkMessengerRuntime | undefined;
    ...
    // Publish the registry and its runtime together. A failed declaration or
    // runtime initialization leaves the last working pair intact; an empty
    // declaration clears the previous runtime instead of retaining stale
  ...
  /**
   * Resolve the registered messenger runtime's out-of-turn delivery surface.
   * This preserves runtime ownership while allowing a subclass to add a
   * product-specific proactive-send API with provider-native addressing.
   */
  async internal_resolveMessengerDeliverySurface(
    channelId: string,
    threadId?: string
  ): Promise<MessengerDeliverySurface | undefined> {
    return this._messengerRuntime?.resolveDeliverySurface(channelId, threadId);
  }
  ...
  // A model stream may end on abort before its tool promises settle. These
  // live operations, including cleanup, remain owned until execute finishes.
  private _activeToolExecutions = new Set<Promise<void>>();
  ...
  // Vendor divergence: upstream's `getSkillScriptRunner()` hook is removed
  // with the Codemode-backed `run_skill_script` tool it feeds (2026-08-16
  // "Active Think and skill barrels no longer reach Codemode").
  ...
  /**
   * Select the inference model before tools and persisted history are rendered.
   * Override for asynchronous model resolution or to snapshot model-dependent
   * rendering policy. Runs for each inference attempt, including continuations.
   * Returning nothing keeps getModel()'s existing resolution point unchanged.
   */
  prepareModel(): ThinkModel | void | Promise<ThinkModel | void> {}
  ...
  private async _prepareInferenceInvocation(
    ...
    const preparedModel = await this.prepareModel();
    const workspaceTools = createWorkspaceTools(this.workspace, {
      bash: this.workspaceBash,
      acceptsImages: this.workspaceAcceptsImages,
      acceptsPdfs: this.workspaceAcceptsPdfs
    });
        ...
        try {
          ...
          const runAction = async () => {
            ...
            const output = await Promise.race([
              this._trackToolExecution(() =>
                executeAction(input, actionContext)
              ),
              abortPromise
            ]);
  ...
  private _beginToolExecution(): () => void {
    let resolve!: () => void;
    const promise = new Promise<void>((settled) => {
      resolve = settled;
    });
    this._activeToolExecutions.add(promise);
    return () => {
      this._activeToolExecutions.delete(promise);
      resolve();
    };
  }

  private async _trackToolExecution<T>(
    execute: () => T | PromiseLike<T>
  ): Promise<T> {
    const settle = this._beginToolExecution();
    try {
    ...
    for (const [toolName, originalTool] of Object.entries(tools)) {
      ...
      const wrappedExecute = isStreamingExecute
        ? (input: unknown, options: ToolDecisionOptions) =>
            ...
            (async function* () {
              const settle = self._beginToolExecution();
              try {
        ...
        : (input: unknown, options: ToolDecisionOptions): Promise<unknown> =>
            self._trackToolExecution(async () => {
              options.abortSignal?.throwIfAborted();
              const resolved = await self._resolveToolCallDecision(
                toolName,
                input,
  ...
  private async _admitTurn<T>(
    ...
    // A facet lease is an asynchronous parent RPC. Bind admission now, not
    // after that reply, so a lease acquired before Stop cannot admit new work.
    const generation = spec.generation ?? this._turnQueue.generation;
    if (spec.signal?.aborted) return { status: "stale" };
    return this.keepAliveWhile(async () => {
      const turnPromise = this._turnQueue.enqueue(
        spec.requestId,
        async (): Promise<AdmittedQueueResult<T>> => {
          // Child cancellation can abort the signal without resetting this
          // queue. Check before any admitted-turn lifecycle or preparation.
          if (spec.signal?.aborted) return { status: "stale" };
          return {
            status: "completed",
            value: await this._runInsideAdmittedTurnBody(spec, stopEpoch)
          };
        },
        { generation }
      );
            ...
            admittedTurnContext.run(
              {
                ...
                startedAt,
                stopEpoch,
                recoveryRootRequestId: spec.recoveryRootRequestId,
  ...
  async chat(
    ...
    try {
      ...
      const admission = await this._admitTurn({
        ...
        execute: async () => {
          ...
          const acceptedAt = new Date(
            admittedTurnContext.getStore()?.startedAt ?? Date.now()
          );
      ...
      if (admission.status === "stale") await callback.onDone();
    } finally {
      detachExternal();
  ...
  protected getAgentToolMilestones(runId: string): AgentToolMilestone[] {
    this._ensureAgentToolChildRunTable();
    return this.sql<{
      sequence: number;
      name: string;
  ...
  private _readAgentToolChildRun(runId: string): AgentToolChildRunRow | null {
    ...
    const rows = this.sql<AgentToolChildRunRow>`
      SELECT run_id, request_id, stream_id, status, summary, output_json,
             output_schema_json, error_message, started_at, completed_at,
  ...
  private _inspectionFromChildRow<Output>(
    ...
    return {
      ...
      ...(() => {
        const milestones = this.getAgentToolMilestones(row.run_id);
  ...
  private static _progressSnapshotFromRow(
    ...
    try {
      const parsed = JSON.parse(
        row.progress_json
      ) as Partial<AgentToolProgressSnapshot>;
      // Older snapshots only retained the last activity time. New snapshots
      // keep their own timestamp so a later milestone cannot retime the text.
      return { ...parsed, at: parsed.at ?? row.last_signal_at };
    } catch {
    ...
    if (!this._agentToolProgressEmitterInstance) {
      this._agentToolProgressEmitterInstance = new AgentToolProgressEmitter({
        ...
        persistSnapshot: (runId, snapshot, at) => {
          ...
          this.sql`
            UPDATE cf_agent_tool_child_runs
            SET progress_json = ${JSON.stringify({ ...snapshot, at })},
                last_signal_at = ${at}
            WHERE run_id = ${runId}
          `;
        },
  ...
  protected formatDetachedCompletion(
    ...
    const label = `Background task "${run.display?.name ?? run.agentType}" (run ${run.runId})`;
  ...
  async startAgentToolRun(
    input: unknown,
    // `output.schema` (vendor seam; see docs/upstream-diff.md) opts the child's
    ...
    const outputSchemaJson = options.output
      ? JSON.stringify(options.output.schema)
      : null;
    if (outputSchemaJson === undefined) {
      throw new TypeError("Agent tool output schema must be JSON-serializable");
    }
      ...
      try {
        ...
        const result = await this._runProgrammaticMessagesTurn(
          ...
          [this.formatAgentToolInput(input)],
          {
            signal: controller.signal,
            shouldApplyMessages: () =>
              !controller.signal.aborted && !this._isStoppedRecovery(),
            trigger: "agent-tool",
            ...(options.output && {
              workflowPrompt: {
                workflow: {
                ...
                output: { schema: options.output.schema }
              },
              captureOutput: true
            })
          }
        ...
        const output =
          result.output !== undefined
            ? result.output
            : this.getAgentToolOutput(options.runId);
  ...
  async cancelAgentToolRun(
    runId: string,
    reason?: unknown
  ): Promise<{ childStillRunning: false }> {
    ...
    if (row?.status === "completed" || row?.status === "error") {
      ...
      // still executing on this dedicated child facet.
      this._aborts.destroyAll(reason);
      await settled;
      return { childStillRunning: false };
    }
    // Commit before signalling or awaiting: a late start RPC and a recovery
    // after eviction both read the same terminal row, including cancel-before-start.
    const now = Date.now();
    this.sql`
      INSERT INTO cf_agent_tool_child_runs (run_id, status, error_message, started_at, completed_at)
      VALUES (${runId}, 'aborted', ${reason instanceof Error ? reason.message : reason === undefined ? null : String(reason)}, ${now}, ${now})
      ON CONFLICT(run_id) DO UPDATE SET status = 'aborted',
        error_message = excluded.error_message, completed_at = COALESCE(completed_at, excluded.completed_at)
      WHERE status NOT IN ('completed', 'error')
    `;
    this._agentToolAbortControllers.get(runId)?.abort(reason);
    this._aborts.destroyAll(reason);
    // A recovered turn re-runs outside `startAgentToolRun`, so it has no entry
    // in `_agentToolAbortControllers`. This facet is dedicated to one run, so
    // aborting every active submission tears that recovery down too (#1630).
  ...
  async inspectAgentToolRun(
    ...
    // No await after reconciliation: metadata, stored chunks and the exclusive
    // live cursor describe one point in this child's stream.
    return {
      ...this._inspectionFromChildRow(row, this.getAgentToolOutput(runId)),
      ...(options?.includeChunks
        ? {
            replay: {
              chunks: this._readAgentToolChunks(row),
              cursor: {
                epoch: this._agentToolLiveEpoch,
                sequence:
                  this._agentToolLiveSequences.get(runId) ??
                  (row.completed_at === null ? 0 : Number.MAX_SAFE_INTEGER)
              }
            }
          }
        : {})
    ...
    if (recoveredTurn) {
      const output =
        row?.output_json == null
          ...
          : Think._parseAgentToolOutput(row.output_json);
  ...
  async getAgentToolChunks(
    ...
    const row = this._readAgentToolChildRun(runId);
    return row ? this._readAgentToolChunks(row, options?.afterSequence) : [];
  }

  private _readAgentToolChunks(
    row: AgentToolChildRunRow,
    afterSequence = -1
  ): AgentToolStoredChunk[] {
    // A live or recovered turn has a request id before its finalizer writes
    // stream_id. Prefer that request's current stream over the previous turn.
    //
    // Vendor divergence (repair): 0.23 deleted `cf_ai_chat_stream_metadata`
    // — chat streams are rows in the shared `agents/streams` log now. The
    // lookup is the same "newest stream for this request, whatever its
    // status", through `ResumableStream`'s own accessor.
    const streamId =
      (row.request_id
        ? this._resumableStream.latestStreamInfoForRequest(row.request_id)?.id
        : undefined) ?? row.stream_id;
    if (!streamId) return [];
  ...
  async tailAgentToolRun(
    ...
    const detach = () => {
      ...
      if (finish) {
        const closers = self._agentToolClosers.get(runId);
        closers?.delete(finish);
        if (closers?.size === 0) self._agentToolClosers.delete(runId);
      }
      if (forward) {
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        finish = () => {
          finished = true;
          // A child can finish while inspection reconciles or crosses RPC.
          // Its saved backlog still belongs to this reader before EOF.
          if (!draining) close?.();
        };
        ...
        // Stored and live positions have different epochs. Progress and
        // milestones advance the live producer without taking a stored index.
        const seen = new Map<string, number>();
        const emit = (chunk: AgentToolStoredChunk) => {
          if (closed) return;
          if (chunk.cursor) {
            const { epoch, sequence } = chunk.cursor;
            if (sequence <= (seen.get(epoch) ?? -1)) return;
            seen.set(epoch, sequence);
          }
          try {
            controller.enqueue(
        ...
        // Subscribe before reading the atomic snapshot and buffer all arrivals
        // until its exclusive live boundary tells us which it already contains.
        ...
        try {
          ...
          if (!replay?.cursor)
            throw new Error(
              "Child inspection did not provide an atomic replay cursor"
            );
          for (const chunk of replay.chunks) {
            if (closed) return;
            if (options?.afterCursor) {
              if (
                chunk.cursor?.epoch === options.afterCursor.epoch &&
                chunk.cursor.sequence <= options.afterCursor.sequence
              )
                continue;
            } else if (chunk.sequence <= (options?.afterSequence ?? -1)) {
              continue;
            }
            emit(chunk);
          }
          ...
          for (const chunk of pending) {
            // Progress/milestones are persisted out of band, not in the
            // resumable chunk list. Preserve actual arrivals without turning
            // an old snapshot into a fresh producer progress notification.
            let outOfBand = false;
            try {
              const body = JSON.parse(chunk.body) as {
                type?: string;
                transient?: boolean;
              };
              outOfBand =
                body.transient === true ||
                body.type === AGENT_TOOL_PROGRESS_PART ||
                body.type === AGENT_TOOL_MILESTONE_PART;
            } catch {
              // Malformed frames remain opaque to this transport boundary.
            }
            if (
              !outOfBand &&
  ...
  private async _emitSubmissionStatus(
    ...
    if (inspection.status === "error" && inspection.error) {
      ...
      // The error is a string; keep it in the message so single-string log
      // surfaces (the extension error page) don't flatten the diagnostic to
      // "[object Object]".
      console.error(`[Think] Submission failed: ${inspection.error}`, {
        submissionId: inspection.submissionId,
        requestId: inspection.requestId,
  ...
  async submitMessages(
    ...
    // Persist the channel and turn metadata on the user messages so the drained
    // turn re-resolves both from history (the model turn runs later in the
    // submission drain).
    messages = this._stampChannel(
      messages,
      options?.channel,
      options?.metadata
    );
  ...
  private _markPendingSubmissionsSkipped(
    includeUnappliedRunning = false
  ): ThinkSubmissionRow[] {
    this._ensureSubmissionTable();
  ...
  private async _runProgrammaticMessagesTurn(
    ...
    const admission = await this._admitTurn({
      ...
      requestId,
      generation: epoch,
      // saveMessages retains accepted input even when its model signal is
      // already aborted. Link that signal only after the history write below.
      continuation: false,
    ...
    if (admission.status === "stale") status = "skipped";
  ...
  protected async continueLastTurn(
    ...
    if (admission.status === "stale") {
      status = options?.signal?.aborted ? "aborted" : "skipped";
    }
  ...
  private async _handleChatRequest(
    ...
    try {
      ...
      messagesPersisted = true;
      this._preStream.markPersisted(requestId, connection);
  ...
  protected resetTurnState(preservePartial = false): void {
    ...
    const skippedSubmissions =
      this._markPendingSubmissionsSkipped(preservePartial);
    ...
    if (!preservePartial) {
      // Clear abandons this history. Stop retains accepted outcomes, so their
      // full-message writes must keep sharing the existing serializer (#1649).
  ...
  /** Stop admitted work and owned children, retaining history and unsent inputs. */
  async stopCurrentWork(): Promise<void> {
    ...
    const submissions = this.sql<{ submission_id: string }>`
      SELECT submission_id FROM cf_think_submissions WHERE status = 'running'
    `.map((row) => this.cancelSubmission(row.submission_id, "Stopped by user"));
    const result = await this._awaitWithDeadline(
      Promise.all([
        this.cancelAgentTools("Stopped by user"),
        settled,
        Promise.all(submissions),
        this._setChatRecovering(false)
      ]),
      Date.now() + this._resolveChatRecoveryConfig().stableTimeoutMs
    );
    if (result === TIMED_OUT)
      throw new Error(
        "Stop was recorded, but current work has not finished stopping. Retry to check cancellation."
      );
    const [children] = result;
    const errors = children.filter((child) => child.status === "rejected");
    if (errors.length)
      throw new Error(
        errors
          .map(
            (child) =>
              `${child.runId}: ${child.error || "Cancellation was not confirmed"}`
          )
          .join("; ")
      );
  }
  ...
  private _annotateActionApprovalChunk(
    ...
    // A durable pause surfaces as a `tool-output-available` chunk whose output
    // is `status: "paused"`, not a `tool-approval-request`. Attach the approval
    // descriptor here so the paused transcript part renders consistently.
  ...
  /**
   * Build the approval descriptor for a paused action output from its persisted
   * pending row. Returns `undefined` for non-paused outputs or missing rows.
   */
  private _descriptorForPausedOutput(
    ...
    if (!executionId?.startsWith(ACTION_PAUSE_ID_PREFIX)) return undefined;
    const row = this._readActionPendingRow(executionId);
  ...
  private async _persistAssistantMessageWithCutover(
    ...
    // metadata"). The cutover writes through Sessions directly, so without
    // this the assistant row would be the one message Think persists without
    // its own `createdAt`, and every later read would fall back to the SQL
    // column — moving the timestamp the client was already shown. An existing
    // row's value still wins, so a continuation re-persisting the same
    // assistant id keeps the original.
    ...
    try {
      this._resumableStream.cutover(
        streamId,
        () => {
          after = sync.upsert(stamped as SessionMessage, {
            parentId,
            source: "server"
          }).after;
        },
  ...
  private async _reconcileAndPersistIncoming(
    ...
    // Vendor divergence: one acceptance instant for the whole batch, so every
    // message this request introduces carries the same durable creation time
   ...
   *
   * Vendor divergence (2026-08-23 "Think keeps completed assistant content
   * server-authoritative", 2026-08-12 "Session and turn timing reach chat
   * metadata"): completed assistant content stays server-owned — the client
   * may be replaying an older streamed snapshot — server-written reserved
   * metadata is restored from the stored row rather than dropped, and the
   * browser's own timing is stripped at intake.
   */
  private async _persistIncomingMessage(
    ...
    const content =
      stored?.role === "assistant" && resolved.role === "assistant"
        ...
        : resolved;
    const storedMetadata = stored?.metadata as
      | Record<string, unknown>
      | undefined;
    const reservedMetadata = Object.fromEntries(
      RESERVED_MESSAGE_METADATA_KEYS.flatMap((key) =>
        storedMetadata && key in storedMetadata
          ? [[key, storedMetadata[key]]]
          : []
      )
    );
    // The stored row's creation time wins on an update, exactly as
    // `_updateMessageInHistory` would apply it — computed here so the skip
    // compare below sees the form that would actually be written.
    const toWrite = withMessageCreatedAt(
      {
        ...content,
        metadata: {
          ...reservedMetadata,
      ...
      // Never the client's own value: browser-supplied timing is stripped at
      // intake and replaced by the server's acceptance instant.
      (stored && messageCreatedAt(stored)) ?? acceptedAt
    );
    if (
      stored &&
      canonicalJson(stored) === canonicalJson(sanitizeMessage(toWrite))
    ) {
      return;
    }
    await this._upsertMessageInHistory(toWrite);
  }

  /**
   * Vendor divergence: strip server-written fields from a client-supplied
   * message at intake. {@link RESERVED_MESSAGE_METADATA_KEYS} are server-written
   * turn context (stamped by `_stampChannel`) that hooks and recovery
   * re-resolve and trust, so a client must never be able to forge them; the
   * top-level `createdAt` is the browser's own timing, which the persistence
   * boundary replaces with the server's (2026-08-12). Sessions' own
   * `source: "client"` strip is not used here — it would also drop the stored
   * reserved values `_persistIncomingMessage` restores.
   */
  private _stripReservedMessageFields(msg: UIMessage): UIMessage {
    const { createdAt: _createdAt, ...message } = msg as UIMessage & {
      createdAt?: unknown;
    };
    const metadata = message.metadata as Record<string, unknown> | undefined;
    if (
  ...
  protected _enqueueInteractionApply(
    ...
    const run = async (): Promise<boolean> => {
      try {
        await this._interactionApplyTail;
      } catch {}
      await apply();
      return true;
    };
  ...
  /**
   * List parked `kind: "durable-pause"` actions carrying their
   * {@link ActionApprovalDescriptor}. Pass an `executionId` to scope to one.
   *
   * Client-callable.
   */
  ...
  /** Approve a parked action and continue the chat with its result. */
  ...
  private async _approveActionPause(executionId: string): Promise<unknown> {
    ...
    // Approval runs outside the original turn. Own its cancellation and
    // lifetime before action lookup, which is itself allowed to be async.
    const stopEpoch = this._stopEpoch;
    const abortSignal = this._aborts.getSignal(executionId);
    const settle = this._beginToolExecution();
    try {
      const action = await this._findRegisteredAction(row.action_name);
      let output: unknown;
      if (abortSignal?.aborted) {
        output = actionErrorEnvelope(abortSignal.reason);
      } else if (!action) {
        output = {
          status: "error",
          executionId,
        ...
        output = await this._runApprovedActionPause(
          action,
          row,
          input,
          abortSignal
      ...
      await this._applyExecutionOutcome(
        executionId,
        output,
        stopEpoch,
        abortSignal
      ...
      this._aborts.remove(executionId);
      settle();
    }
  }
    ...
    try {
      ...
      const runAction = async () => {
        ...
        const out = await Promise.race([
          this._trackToolExecution(() => executeAction(input, ctx)),
          abortPromise
        ]);
  ...
  private async _applyExecutionOutcome(
    ...
    if (stopEpoch !== this._stopEpoch || abortSignal?.aborted) return true;
    ...
    while (true) {
      if (
        (await this._awaitWithDeadline(
          this._waitForAgentToolRunRecovery(),
          deadline
        )) === TIMED_OUT
      ) {
        return false;
  ...
  private async _chatRecoveryProgressMarker(): Promise<number> {
    ...
    //
    // Vendor divergence: the seed runs as a synchronously-started async
    // function rather than upstream's `.then(...)` pair. `seedProgress()`
    // WRITES (`cf_agents_chat_progress`), and a detached `.then` continuation
    // reaches storage without this actor's input lock. Memoization, failure
    // eviction and ordering are otherwise identical.
    this._progressSeed ??= (async () => {
      try {
        this._resumableStream.seedProgress(
          await readChatRecoveryProgress(this.ctx.storage)
        );
      } catch (error) {
  ...
  /**
   * `true` when Stop invalidated this recovery, after sealing its incident as a
   * benign cancellation. Stop itself owns terminalizing the submission.
   */
  private async _cancelRecoveryIfStopped(
    stopEpoch: number,
    incidentId: string | undefined
  ): Promise<boolean> {
    if (!this._isStoppedRecovery(stopEpoch)) return false;
    await this._updateChatRecoveryIncident(incidentId, "skipped", "cancelled");
    return true;
  }
  ...
  protected override async _handleInternalFiberRecovery(
    ...
    return this._chatRecoveryEngine().handleChatFiberRecovery(ctx, {
      ...
      isRecoveryCancelled: (snapshot) =>
        this._isStoppedRecovery(snapshot?.stopEpoch ?? 0),
  ...
  private async _dispatchRecoveredThinkTurn(
    ...
    if (
      await this._cancelRecoveryIfStopped(
        input.snapshot?.stopEpoch ?? 0,
   ...
   *   leaving the incident `scheduled` with the error for observability —
   *   vendor divergence, 2026-07-31 "Recovery ordering preserves retryable
   *   Agent Tool turns": the incident must not be mislabeled terminal while
   *   the platform still owns a retry) so the current attempt (the driving
   *   Task run, or the routed one-shot schedule row) is preserved and the
   *   platform re-runs recovery once it is healthy again — the turn can
  ...
  skipChatRequest(connection: Connection, requestId: string): void {
    ...
    // fires before the successor claims its request id, so releasing here would
    // race that claim and cut a parked client loose right before the successor
    // streams. Leave it parked: the successor flushes it on
  ...
  private _settlePreStreamTurn(
    ...
    const persisted = this._preStream.isPersisted(requestId);
    const idle = this._preStream.settle(requestId);
    if (persisted && !this._resumableStream.hasRequest(requestId)) {
      this._recordCompletedRequest(requestId);
    }
  ...
  /**
   * Vendor divergence: retain an empty completed receipt for a turn that
   * settled without ever opening a stream, so a replayed request id is
   * recognized instead of running inference twice (2026-08-14 "Unacknowledged
   * chat requests replay across route changes").
   *
   * At 0.22 this also armed the stream-cleanup alarm. In 0.23 finished stream
   * rows are reclaimed by the next `ResumableStream.start()` on this agent, so
   * there is no alarm to arm — and the receipt lives until that next start
   * rather than for the old ten-minute window.
   */
  private _recordCompletedRequest(requestId: string): void {
    this._resumableStream.recordCompletedRequest(requestId);
  }
  ...
  private _fireAutoContinuation(): void {
    ...
    this._admitTurn({
      ...
      execute: async () => {
        ...
        if (abortSignal?.aborted || current?.requestId !== requestId) return;
        current.pastCoalesce = true;
        try {
  ...
  private async _recordTerminalChatStatus(
    ...
    // A terminal turn resolves its own recovery, never a newer request's.
    // Wake/exhaustion paths supply the root explicitly outside admitted ALS.
    const turn = admittedTurnContext.getStore();
    await this._setChatRecovering(
      false,
      recoveryRootRequestId ??
        (turn?.agent === this ? turn.recoveryRootRequestId : undefined) ??
        requestId
    );
  }
