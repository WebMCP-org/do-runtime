...
export interface WorkspaceToolsOptions {
  ...
  /**
   * Whether the model reads image parts. Enabled by default; pass `false`, or a
   * thunk for a host whose model can change mid-conversation, to render an
   * image read as a note naming the file. See {@link ReadToolOptions}.
   */
  acceptsImages?: ReadToolOptions["acceptsImages"];
  /**
   * Whether the model reads PDF file parts. Enabled by default; pass `false`,
   * or a thunk, to render a PDF read as a note naming the file. See
   * {@link ReadToolOptions}.
   */
  acceptsPdfs?: ReadToolOptions["acceptsPdfs"];
}
...
export function createWorkspaceTools(
  ...
  const tools: WorkspaceTools = {
    read: createReadTool({
      ops: workspaceReadOps(workspace),
      acceptsImages: options.acceptsImages,
      acceptsPdfs: options.acceptsPdfs
    }),
...
// The raster formats OpenAI accepts as `input_image`. Anything else claiming
// image/* (SVG, TIFF, a mislabeled stat mimeType) has no inline form the model
// reads.
...
export interface ReadToolOptions {
  ops: ReadOperations;
  /**
   * Whether the model this tool renders for reads image parts. Consulted at
   * render time rather than read time, because `toModelOutput` re-renders a
   * stored result on every later request: a model swap has to change what the
   * next request carries, without rewriting the transcript behind it.
   */
  acceptsImages?: boolean | (() => boolean | Promise<boolean>);
  /**
   * Whether the model this tool renders for reads PDF `file-data` parts.
   * Same render-time contract as {@link acceptsImages}.
   */
  acceptsPdfs?: boolean | (() => boolean | Promise<boolean>);
}

export function createReadTool(options: ReadToolOptions): Tool {
  const { ops, acceptsImages = true, acceptsPdfs = true } = options;
  ...
  return tool({
    ...
    toModelOutput: async ({
      ...
      // A model with no image input does not refuse an image part: it bills the
      // base64 as input on every re-render and then answers from a picture it
      // invented. Naming the file is the honest render, and returning before
      // the encode keeps a megabyte of base64 out of every later request. The
      // note states the fact and stops there — `read` takes a path and a line
      // window, so any recovery it named would send the model back to this same
      // branch.
      if (replayOutput.kind === "image") {
        const modelReadsImages =
          typeof acceptsImages === "function"
            ? await acceptsImages()
            : acceptsImages;
        if (!modelReadsImages) {
          return {
            type: "error-text",
            value: `${note} This model has no image input, so the picture was not sent.`
          };
        }
      }

      // A PDF `file-data` part fares no better on an endpoint without PDF
      // input: a gateway that rejects the part fails every later request this
      // render rides in, and one that strips it leaves the model a note naming
      // a document it never received — which reads as an instruction to
      // reconstruct the document by hand. Same honest render as an image.
      if (replayOutput.kind === "file") {
        const modelReadsPdfs =
          typeof acceptsPdfs === "function" ? await acceptsPdfs() : acceptsPdfs;
        if (!modelReadsPdfs) {
          return {
            type: "error-text",
            value: `${note} This model has no PDF input, so the document was not sent.`
          };
        }
      }
      ...
      if (replayOutput.kind === "image") {
        // Only the raster formats the model actually reads have an inline
        // `image-data` form. Anything else claiming image/* (SVG, TIFF, a
        // mislabeled stat mimeType) has none, and toModelOutput re-renders this
        // part on every later request, so sending one would fail the
        // conversation from then on.
        if (!MODEL_IMAGE_MEDIA_TYPES.has(replayOutput.mediaType)) {
          return {
            type: "error-text",
            value:
              `${note} The model reads PNG, JPEG, WebP, and GIF images inline; ` +
              `re-render or convert this file to one of those to view it.`
          };
        }
        return {
          type: "content",
