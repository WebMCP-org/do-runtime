...
describe("Think agent tools", () => {
  it("rejects Stop when a real child rejects cancellation with an empty diagnostic", async () => {
    const parent = await getAgentByName(
      env.ThinkTestAgent,
      crypto.randomUUID()
    );
    const runId = "empty-cancellation-error";
    const running = parent.runHeldChildForStopForTest(runId);
    try {
      await expect
        .poll(
          async () => (await parent.getAgentToolStopStateForTest(runId))?.status
        )
        .toBe("running");
      await expect(Promise.resolve(parent.stopCurrentWork())).rejects.toThrow(
        "Cancellation was not confirmed"
      );
      expect(await parent.getAgentToolStopStateForTest(runId)).toMatchObject({
        status: "aborted",
        childStillRunning: true
      });

      await parent.allowChildCancellationForTest(runId);
      await parent.stopCurrentWork();
      expect(await parent.getAgentToolStopStateForTest(runId)).toMatchObject({
        status: "aborted",
  ...
  it("does not discard a new stream because the previous stream had a larger offset", async () => {
    const agent = await freshAgent();
    const chunks = await agent.readOverlappingTailForTest(crypto.randomUUID(), {
      epoch: "previous-stream",
      sequence: 999
    });
    const bodies = chunks.map((chunk) => JSON.parse(chunk.body));
    expect(
      bodies
  ...
  it("tails overlapping stored output exactly once when live progress has a different ordinal", async () => {
    const agent = await freshAgent();
    const chunks = await agent.readOverlappingTailForTest(crypto.randomUUID());
    const bodies = chunks.map((chunk) => JSON.parse(chunk.body));
    expect(
    ...
    expect(
      bodies
        .filter((body) => body.type === AGENT_TOOL_PROGRESS_PART)
        .map((body) => body.data.message)
    ).toEqual(["first signal", "second signal"]);
    expect(
      bodies
        .filter((body) => body.type === AGENT_TOOL_MILESTONE_PART)
        .map((body) => body.data.name)
    ).toEqual(["overlap-ready"]);
  });
