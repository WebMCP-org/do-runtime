...
import {
  ...
  type MessengerThinkTarget,
  type NormalizedMessengerDefinition
} from "../messengers";
...
describe("think messengers core", () => {
  it("uses a resolved conversation target without creating a sub-agent", async () => {
    const agent: MessengerThinkTarget = {
      cancelChat: () => true,
      chatWithMessengerDelivery: () => Promise.resolve()
    };
    const [definition] = normalizeMessengers({
      fake: chatSdkMessenger({
        adapter: fakeAdapter(),
        conversation: () => ({ agent, target: "resolved" }),
        provider: "fake",
        userName: "fake_bot",
        verifyWebhook: false
      })
    });
    const runtime = new ThinkMessengerRuntime({}, fakeHost([]));
    ...
    const resolved = await (
      runtime as unknown as {
        ...
        ): Promise<MessengerThinkTarget>;
      }
    ).resolveTarget(definition!, baseEvent);
  ...
  it("marks messenger reply fibers as streaming only when visible text starts", async () => {
    ...
    await deliverMessengerReply({
      ...
      target: {
        ...
        chatWithMessengerDelivery(_message, callback) {
  ...
  it("delivers successful replies with active messenger context and overflow chunks", async () => {
    ...
    await deliverMessengerReply({
      ...
      target: {
        ...
        chatWithMessengerDelivery(_message, callback, context) {
          seenContext = context;
          callback.onEvent(
            JSON.stringify({ type: "text-delta", delta: "hello" })
          );
  ...
  it("reconciles changed and removed messenger declarations", async () => {
    const agent = await getAgentByName(
      env.ThinkMessengerRouteTestAgent,
      `route-reconcile-${crypto.randomUUID()}`
    );
    const request = (token?: string) =>
      agent.fetch("https://example.com/messengers/fake/webhook", {
        method: "POST",
        headers: token ? { "x-test-messenger-token": token } : undefined
      });

    await agent.setMessengerToken("first");
    await expect((await request("first")).text()).resolves.toBe("messenger");
    expect((await request("wrong")).status).toBe(401);

    await agent.setMessengerToken("second");
    expect((await request("first")).status).toBe(401);
    await expect((await request("second")).text()).resolves.toBe("messenger");

    await agent.setMessengerToken(null);
    await expect((await request("second")).text()).resolves.toBe("fallback");
  });
});
