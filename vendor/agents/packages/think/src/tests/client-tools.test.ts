...
describe("Think — auto-continuation", () => {
  ...
  it.each(["stop", "clear"] as const)(
    "keeps accepted tool-result writes coherent across %s",
    async (operation) => {
      const room = crypto.randomUUID();
      const agent = await freshAgent(room);
      const { ws } = await connectWS(room);
      const messageId = "assistant-reset-apply";
      await agent.persistToolCallMessage([
        makeUserMessage("use both tools"),
        {
          id: messageId,
          role: "assistant",
          parts: ["tc-a", "tc-b"].map((toolCallId) => ({
            type: "tool-client_action" as const,
            toolCallId,
      ...
      await agent.holdNextMessageUpdateForTest();
      try {
        await agent.deliverToolResultForTest("tc-a", "result A");
        await expect.poll(() => agent.isMessageUpdateHeldForTest()).toBe(true);

        if (operation === "stop") await agent.stopCurrentWork();
        else await agent.clearMessages();
        await agent.deliverToolResultForTest("tc-b", "result B");

        // A later result must not commit a competing snapshot while A's real
        // storage write is held. Clear instead removes the target entirely.
        const heldHistory = await agent.getStoredToolStatesForTest();
        if (operation === "stop") {
          expect
            .soft(
              heldHistory.find((message) => message.id === messageId)?.parts
            )
            .toEqual(
        ...
        await agent.releaseMessageUpdateForTest();
        const history = await agent.getStoredToolStatesForTest();
        if (operation === "stop") {
          expect(
            history.find((message) => message.id === messageId)?.parts
          ).toEqual(
            expect.arrayContaining([
              expect.objectContaining({
                toolCallId: "tc-a",
        ...
        expect(await agent.getResponseLog()).toEqual([]);
      } finally {
        await agent.releaseMessageUpdateForTest();
        await closeWS(ws);
      }
