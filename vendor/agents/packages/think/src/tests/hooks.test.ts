...
import { acknowledgeStreamResume } from "./resume-handshake";

const MSG_CHAT_REQUEST = "cf_agent_use_chat_request";
const MSG_CHAT_RESPONSE = "cf_agent_use_chat_response";
const MSG_TOOL_APPROVAL = "cf_agent_tool_approval";

async function connectWS(agentClass: string, room: string, pathSuffix = "") {
  const slug = agentClass
    .replace(/([a-z])([A-Z])/g, "$1-$2")
    .replace(/([A-Z]+)([A-Z][a-z])/g, "$1-$2")
    .toLowerCase();
  const res = await exports.default.fetch(
    `http://example.com/agents/${slug}/${room}${pathSuffix}`,
    { headers: { Upgrade: "websocket" } }
  );
  expect(res.status).toBe(101);
  const ws = res.webSocket as WebSocket;
  expect(ws).toBeDefined();
  acknowledgeStreamResume(ws);
  ws.accept();
  return ws;
}
...
describe("Stop before turn admission", () => {
  it("closes a native chat stopped while onStart is pending without entering preparation", async () => {
    const agent = await freshToolAgent(`stop-callback-${crypto.randomUUID()}`);

    expect(await agent.stopDuringChatStartForTest()).toEqual({
      preparations: 0,
      done: true
    });

    expect((await agent.testChat("fresh request after Stop")).done).toBe(true);
    expect((await agent.continuationAdmissionProbeForTest()).preparations).toBe(
      1
    );
  });

  it("skips a child chat cancelled while its facet lease is pending without losing the user input", async () => {
    const room = `cancel-chat-lease-${crypto.randomUUID()}`;
    const childName = "approval-child";
    const parent = await getAgentByName(env.ThinkAgentToolParent, room);
    await parent.prepareApprovalFacetForTest(childName);
    const childPath = `/sub/think-tools-test-agent/${childName}`;
    const ws = await connectWS("ThinkAgentToolParent", room, childPath);
    const lease = await parent.holdNextFacetLeaseForTest(childName);
    try {
      const done = waitForDone(ws);
      sendChatRequest(ws, "Retain this accepted input after cancellation");
      await vi.waitFor(async () => {
        expect((await parent.facetLeaseProbeForTest(lease)).acquired).toBe(
          true
        );
      });
      await parent.stopApprovalFacetForTest(childName, "cancel-child");
      await parent.releaseFacetLeaseForTest(lease);
      await done;
      await vi.waitFor(async () => {
        expect((await parent.facetLeaseProbeForTest(lease)).disposed).toBe(
          true
        );
      });

      const probe = await parent.approvalFacetProbeForTest(childName);
      expect(probe.preparations).toBe(0);
      expect(probe.actionExecutions).toBe(0);
      const response = await exports.default.fetch(
        `http://example.com/agents/think-agent-tool-parent/${room}${childPath}/get-messages`
      );
      expect(response.status).toBe(200);
      const messages = (await response.json()) as UIMessage[];
      expect(messages).toHaveLength(1);
      expect(messages[0].parts).toEqual([
        { type: "text", text: "Retain this accepted input after cancellation" }
      ]);

      const freshDone = waitForDone(ws);
      sendChatRequest(ws, "fresh request after child cancellation");
      await freshDone;
      expect(
        (await parent.approvalFacetProbeForTest(childName)).preparations
      ).toBe(1);
    } finally {
      await parent.releaseFacetLeaseForTest(lease);
      try {
        await vi.waitFor(async () => {
          const state = await parent.facetLeaseProbeForTest(lease);
          expect(!state.acquired || state.disposed).toBe(true);
        });
      } finally {
        await closeWS(ws);
      }
  ...
  it.each(["stop", "cancel-child"] as const)(
    "%s skips a connected continuation whose facet lease arrives late, preserving fresh work",
    async (operation) => {
      const room = `stop-lease-${operation}-${crypto.randomUUID()}`;
      const childName = "approval-child";
      const parent = await getAgentByName(env.ThinkAgentToolParent, room);
      await parent.prepareApprovalFacetForTest(childName);
      const ws = await connectWS(
        ...
        room,
        `/sub/think-tools-test-agent/${childName}`
      );
      const held: number[] = [];
      const approve = () =>
        ws.send(
      ...
      try {
        ...
        await vi.waitFor(async () => {
          expect(
            (await parent.approvalFacetProbeForTest(childName)).responses
          ).toBe(1);
        });
        const before = await parent.approvalFacetProbeForTest(childName);

        const obsolete = await parent.holdNextFacetLeaseForTest(childName);
        held.push(obsolete);
        approve();
        await vi.waitFor(async () => {
          expect((await parent.facetLeaseProbeForTest(obsolete)).acquired).toBe(
            true
          );
        });
        await parent.stopApprovalFacetForTest(childName, operation);

        const releaseObsolete = async () => {
          await parent.releaseFacetLeaseForTest(obsolete);
          await vi.waitFor(async () => {
            expect(
              (await parent.facetLeaseProbeForTest(obsolete)).disposed
            ).toBe(true);
          });
          const afterStop = await parent.approvalFacetProbeForTest(childName);
          expect(afterStop.preparations).toBe(before.preparations);
          expect(afterStop.actionExecutions).toBe(0);
        };
        // Child cancellation does not reset the continuation state. Let its
        // cancelled callback settle before a fresh approval; approving while
        // it is still pending coalesces into that same request ID.
        if (operation === "cancel-child") await releaseObsolete();

        // Stop resets pending state, so this approval has a different owner
        // even while the old callback is waiting. Hold its real lease too so
        // the old finally must preserve the newer pending continuation.
        const current = await parent.holdNextFacetLeaseForTest(childName);
        held.push(current);
        approve();
        await vi.waitFor(async () => {
          expect((await parent.facetLeaseProbeForTest(current)).acquired).toBe(
            true
          );
        });

        if (operation === "stop") await releaseObsolete();

        const resumed = waitForDone(ws);
        await parent.releaseFacetLeaseForTest(current);
        await resumed;
        await vi.waitFor(async () => {
          expect((await parent.facetLeaseProbeForTest(current)).disposed).toBe(
            true
          );
        });
        const afterApproval = await parent.approvalFacetProbeForTest(childName);
        expect(afterApproval.preparations).toBe(before.preparations + 1);
        expect(afterApproval.actionExecutions).toBe(1);

        const freshDone = waitForDone(ws);
        sendChatRequest(ws, "fresh request after the cancelled lease settled");
        await freshDone;
        expect(
          (await parent.approvalFacetProbeForTest(childName)).preparations
        ).toBe(before.preparations + 2);
      } finally {
        await Promise.all(
          held.map((index) => parent.releaseFacetLeaseForTest(index))
        );
        try {
          await vi.waitFor(
            async () => {
              const leases = await Promise.all(
                held.map((index) => parent.facetLeaseProbeForTest(index))
              );
              expect(
                leases.every((lease) => !lease.acquired || lease.disposed)
              ).toBe(true);
            },
            { timeout: 5000 }
          );
