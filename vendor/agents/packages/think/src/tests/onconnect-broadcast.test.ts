...
describe("Think — onConnect broadcast policy", () => {
  it("replays a duplicate completed request id without admitting a second turn", async () => {
    ...
    await collectMessages(ws, 50);
    const requestId = "think-duplicate-completed";
    ...
    const firstTurnFrames = collectMessages(ws, 100);
    ws.send(requestFrame);
    expect(await firstTurnFrames).toContainEqual(
    ...
    expect((await agent.getBeforeTurnLog()).length).toBe(1);

    const duplicateFrames = collectMessages(ws, 100);
    ws.send(requestFrame);
    expect(await duplicateFrames).toContainEqual({
  ...
  it.each(["success", "error"] as const)(
    "replays a real %s turn that terminalizes before the reconnect ACK",
    async (outcome) => {
      const room = crypto.randomUUID();
      const agent = await freshAgent(room);
      await agent.setStreamChunkDelayForTest(40);

      const turn =
        outcome === "success"
          ? agent.testChat("finish while reconnecting")
          : agent.testChatWithError("reconnect terminal error");
      const active = await agent.waitForActiveResumableStreamForTest();
      expect(active).not.toBeNull();
      if (!active) throw new Error("real turn never opened a resumable stream");

      const { ws } = await connectWS(room);
      const connectMessages = await collectMessages(ws, 50);
      ...
      expect(result.done).toBe(outcome === "success");
      expect(result.error === undefined).toBe(outcome === "success");

      ws.send(
        JSON.stringify({ type: MSG_STREAM_RESUME_ACK, id: active.requestId })
      );
      const resumed = (await collectMessages(ws)).filter(
        (message) => message.type === MSG_CHAT_RESPONSE
      );
      expect(resumed.some((message) => message.replay === true)).toBe(true);
      expect(resumed.at(-1)).toEqual(
        expect.objectContaining({
          type: MSG_CHAT_RESPONSE,
          id: active.requestId,
          done: true,
          ...(outcome === "error" ? { error: true } : { replay: true })
        })
      );
...
describe("Think — terminal replay on reconnect (#1645)", () => {
  ...
  it("eagerly drops the terminal record when a new turn is submitted, before it streams (#1645)", async () => {
    ...
    const submittedFrames = collectMessages(ws);
    ws.send(
      JSON.stringify({
        type: "cf_agent_use_chat_request",
        id: "root-new",
    ...
    expect(await submittedFrames).toContainEqual({
