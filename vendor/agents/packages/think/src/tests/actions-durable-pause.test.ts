...
describe("durable-pause actions (turn-driven, connection-less)", () => {
  it("Stop during action lookup consumes approval without starting a late action or continuation", async () => {
    const agent = await freshPauseAgent(
      `dp-stop-lookup-${crypto.randomUUID()}`
    );
    await agent.useDurablePauseActionForTest();
    expect((await agent.testChat("call pauseAction")).done).toBe(true);
    const [pending] = await agent.listActionPendingForTest();

    const stopped = await agent.stopDurablePauseForTest(
      pending.execution_id,
      "lookup"
    );

    expect(await agent.getDurablePauseExecCount()).toBe(0);
    expect(stopped.acknowledgedWhileHeld).toBe(false);
    expect(stopped.finishedWhenAcknowledged).toBe(true);
    expect(stopped.stable).toBe(true);
    expect(stopped.afterSteps).toBe(stopped.beforeSteps);
    expect(JSON.parse(stopped.outputJson)).toHaveProperty("error");
    expect(await agent.listActionPendingForTest()).toHaveLength(0);
    const messages = (await agent.getStoredMessages()) as UIMessage[];
    const part = messages
      .flatMap((message) => message.parts)
      .find((part) => "toolCallId" in part && part.toolCallId === "dp1");
    expect(part).toMatchObject({
      state: "output-available",
      output: JSON.parse(stopped.outputJson)
    });

    expect(
      (await agent.testChat("new input after stopped approval")).done
    ).toBe(true);
    expect(await agent.getBeforeStepLog()).toHaveLength(
      stopped.beforeSteps + 1
    );
  });

  it("Stop signals an approved action and waits for actual cleanup without resuming the model", async () => {
    const agent = await freshPauseAgent(
      `dp-stop-execute-${crypto.randomUUID()}`
    );
    await agent.useDurablePauseActionForTest();
    expect((await agent.testChat("call pauseAction")).done).toBe(true);
    const [pending] = await agent.listActionPendingForTest();
    ...
    expect(stopped.signalAborted).toBe(true);
    expect(stopped.acknowledgedWhileHeld).toBe(false);
    expect(stopped.finishedWhenAcknowledged).toBe(true);
    expect(stopped.stable).toBe(true);
    expect(stopped.afterSteps).toBe(stopped.beforeSteps);
  ...
  it("Stop on a retained child signals its approved action without rewriting the completed run", async () => {
    const agent = await freshPauseAgent(`dp-stop-child-${crypto.randomUUID()}`);
    await agent.useDurablePauseActionForTest();
    const runId = `pause-run-${crypto.randomUUID()}`;
    await agent.startAgentToolRun("call pauseAction", { runId });
    await vi.waitFor(async () => {
      expect(await agent.getChildRunStatusForTest(runId)).toBe("completed");
    });
    const [pending] = await agent.listActionPendingForTest();
