...
import type { Session, SessionMessage } from "../../think";
...
export class ThinkClientToolsAgent extends Think {
  ...
  private _messageUpdateGate?: {
    wait: Promise<void>;
    release: () => void;
    settled?: Promise<SessionMessage | null>;
  };

  override configureSession(session: Session): Session {
    const updateMessage = session.updateMessage.bind(session);
    session.updateMessage = (message, options) => {
      const gate = this._messageUpdateGate;
      if (gate && !gate.settled) {
        // Hold the real Session write, retaining its SQL and change callbacks.
        gate.settled = gate.wait.then(() => updateMessage(message, options));
        return gate.settled;
      }
      return updateMessage(message, options);
    };
    return session;
  }

  holdNextMessageUpdateForTest(): void {
    let release!: () => void;
    const wait = new Promise<void>((resolve) => {
      release = resolve;
    });
    this._messageUpdateGate = { wait, release };
  }

  isMessageUpdateHeldForTest(): boolean {
    return this._messageUpdateGate?.settled !== undefined;
  }

  async releaseMessageUpdateForTest(): Promise<void> {
    const gate = this._messageUpdateGate;
    this._messageUpdateGate = undefined;
    gate?.release();
    await gate?.settled;
    await this._enqueueInteractionApply(async () => {});
  }

  async deliverToolResultForTest(
    toolCallId: string,
    output: string
  ): Promise<void> {
    const connection = [...this.getConnections()][0];
    if (!connection) throw new Error("Tool result requires a connected client");
  ...
  async getLastTurnMessagesJson(): Promise<string> {
  ...
  async getStoredToolStatesForTest() {
    return (await this.session.getHistory()).map((message) => ({
      id: message.id,
      parts: message.parts
        .filter(
          (part) =>
        ...
        .map((part) => ({
          ...
          state: part.state,
          output: z.string().optional().parse(part.output)
        }))
    }));
  }
