...
export class ThinkTestAgent extends Think {
  ...
  private _tailOverlap: {
    advance: () => void;
    whenAdvanced: Promise<void>;
    finish: () => void;
    whenFinished: Promise<void>;
    snapshotRead: () => void;
  } | null = null;

  /** Delay the adapter read until real model output overlaps its live tail. */
  private async _waitForTailOverlapSnapshot(runId: string): Promise<void> {
    if (!this._tailOverlap) return;
    this._tailOverlap.advance();
    const deadline = Date.now() + 2000;
    while (Date.now() < deadline) {
      const chunks = await super.getAgentToolChunks(runId);
      if (chunks.some((chunk) => chunk.body.includes("overlap-once"))) return;
      await new Promise((resolve) => setTimeout(resolve, 5));
    }
    throw new Error("Model output did not reach the replay snapshot");
  }

  override async inspectAgentToolRun(
    runId: string,
    options?: { includeChunks?: boolean }
  ): ReturnType<Think["inspectAgentToolRun"]> {
    if (options?.includeChunks) await this._waitForTailOverlapSnapshot(runId);
    const inspection = await super.inspectAgentToolRun(runId, options);
    if (options?.includeChunks) {
      // Existing attach-race fixtures inject AFTER this atomic snapshot.
      if (this._attachRaceInjection || this._progressInjection) {
        await this.getAgentToolChunks(runId);
      }
      this._tailOverlap?.snapshotRead();
    }
    return inspection;
  }

  async readOverlappingTailForTest(
    runId: string,
    ...
    const whenAdvanced = new Promise<void>((resolve) => {
      advance = resolve;
    });
    ...
    try {
      await this.startAgentToolRun("overlap a real model with its replay", {
        runId
      });
      ...
      const drain = (async () => {
        const chunks: AgentToolStoredChunk[] = [];
        const decoder = new TextDecoder();
        let body = "";
        for await (const bytes of stream as unknown as ReadableStream<Uint8Array>) {
          body += decoder.decode(bytes, { stream: true });
        }
        body += decoder.decode();
        for (const line of body.trim().split("\n")) {
          if (line) chunks.push(JSON.parse(line) as AgentToolStoredChunk);
        }
        return chunks;
      })();
      await snapshot;
      ...
      advance();
      finish();
      this._tailOverlap = null;
      await this.cancelAgentToolRun(runId, "tail fixture cleanup");
    }
  }
  ...
  async holdAgentToolModelForTest(): Promise<void> {
    this._holdAgentToolModel = true;
  }

  setCancellationErrorForTest(error: string | null): void {
    this._cancellationErrorForTest = error;
  }

  override async cancelAgentToolRun(
    runId: string,
    reason?: unknown
  ): ReturnType<Think["cancelAgentToolRun"]> {
    if (this._cancellationErrorForTest !== null)
      throw new Error(this._cancellationErrorForTest);
    return super.cancelAgentToolRun(runId, reason);
  }

  async runHeldChildForStopForTest(runId: string): Promise<RunAgentToolResult> {
    const child = await this.subAgent(ThinkTestAgent, runId);
    await child.holdAgentToolModelForTest();
    await child.setCancellationErrorForTest("");
    return this.runAgentTool(ThinkTestAgent, { runId, input: "held child" });
  }

  async allowChildCancellationForTest(runId: string): Promise<void> {
    const child = await this.subAgent(ThinkTestAgent, runId);
    await child.setCancellationErrorForTest(null);
  }

  async getAgentToolStopStateForTest(runId: string) {
    const run = await this.inspectAgentTool(runId);
    return run
      ? { status: run.status, childStillRunning: run.childStillRunning }
      : null;
  }
  ...
  /**
   * Start a deliberately slow agent-tool turn, wait until its resumable stream
   * has persisted an early chunk, then read the public backlog while the child
   * row is still `running`.
   */
  async readRunningAgentToolBacklogForTest(runId: string): Promise<{
    status: string;
    streamId: string;
    storedChunks: AgentToolStoredChunk[];
    visibleChunks: AgentToolStoredChunk[];
  }> {
    this._streamChunkDelayMs = 50;
    try {
      await this.startAgentToolRun("read the early stream backlog", { runId });
      const deadline = Date.now() + 2000;
      while (Date.now() < deadline) {
        const row = this["_readAgentToolChildRun"](runId);
        const requestId = row?.request_id ?? undefined;
        if (row?.status === "running" && requestId) {
          const streamId = this["_resumableStream"]
            .getAllStreamMetadata()
            .find((metadata) => metadata.request_id === requestId)?.id;
          if (streamId) {
            this["_resumableStream"].flushBuffer();
            const storedChunks = this["_resumableStream"]
            ...
            if (storedChunks.length > 0) {
              return {
                ...
                streamId,
                storedChunks,
                visibleChunks: await super.getAgentToolChunks(runId)
              };
            }
          }
        }
      ...
      throw new Error("Timed out waiting for an early agent-tool stream chunk");
    } finally {
      this._streamChunkDelayMs = null;
      await this.cancelAgentToolRun(runId, "test cleanup");
    }
  }
  ...
  private _releaseMidStream: (() => void) | null = null;
  private _midStreamGate: Promise<void> | null = null;

  /**
   * Park the model stream mid-content, with the first chunks already stored
   * and broadcast, so a resume ACK runs against a live stream that has more
   * to come.
   *
   * Vendor divergence (adapted): this used to hold the per-chunk
   * recovery-progress storage read (`cf:chat-recovery:progress`) — the await
   * the 2026-09-11 "Atomic replay storage and live chunk delivery" entry
   * closed over. Agents 0.23 derives the recovery marker from the stream log
   * and writes nothing per chunk, so `_storeChunkDurably` is now fully
   * synchronous and that window cannot be entered at all: store and live
   * broadcast are one uninterrupted step. The cases below still assert the
   * same user-visible guarantee — each chunk reaches the client exactly once
   * across a mid-stream resume — held at the yield that remains, the model
   * stream itself.
   */
  holdStreamAfterFirstChunkForTest(): void {
    this._midStreamGate = new Promise<void>((resolve) => {
      this._releaseMidStream = resolve;
    });
    this._midStreamEntered = false;
  }

  private _midStreamEntered = false;

  isStreamHeldAfterFirstChunkForTest(): boolean {
    return this._midStreamEntered && this._releaseMidStream !== null;
  }

  releaseStreamAfterFirstChunkForTest(): void {
    this._releaseMidStream?.();
    this._releaseMidStream = null;
    this._midStreamGate = null;
  }
  ...
  async waitForActiveResumableStreamForTest(
    timeoutMs = 2000
  ): Promise<{ requestId: string; streamId: string } | null> {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      const active = this._resumableStream
        .getAllStreamMetadata()
        .find((stream) => stream.status === "streaming");
      if (active) return { requestId: active.request_id, streamId: active.id };
      await new Promise((resolve) => setTimeout(resolve, 5));
    }
    return null;
  }
  ...
  async testChatWithThrowingDoneCallback(message: string): Promise<string> {
    const cb: StreamCallback = {
      onStart() {},
      onEvent() {},
      onDone() {
  ...
  async runProviderReplayContinuationForTest() {
    const toolCallId = "replayed-tool";
    ...
    // @ts-expect-error -- test seam drives Think's private persistence owner end to end.
    await this._persistAssistantMessage(prior);
    ...
    const chunks = [
      ...
      { type: "tool-output-available", toolCallId, output: { answer: 42 } },
      { type: "text-start", id: "new-text" },
      { type: "text-delta", id: "new-text", delta: "continued" },
      { type: "text-end", id: "new-text" }
    ];
    ...
    // @ts-expect-error -- test seam drives Think's private stream owner end to end.
    await this._streamResult(crypto.randomUUID(), result, undefined, {
      continuation: true
    });
    return (await this.syncMessagesFromStorage()).flatMap((message) =>
  ...
  async abortWhileStructuredOutputSettlesForTest(): Promise<{
    returnedStatus: string;
    storedStatus: unknown;
    hookStatus: string | undefined;
  }> {
    let noteOutputRead!: () => void;
    const outputRead = new Promise<void>((resolve) => {
      noteOutputRead = resolve;
    });
    ...
    const chunks = [
      { type: "text-start", id: "settling-output" },
      { type: "text-delta", id: "settling-output", delta: "Settled text" },
      { type: "text-end", id: "settling-output" }
    ];
    const result = {
      get output() {
        noteOutputRead();
    ...
    )._streamResult(crypto.randomUUID(), result, controller.signal, {
      captureOutput: true
    });

    await outputRead;
    controller.abort();
    releaseOutput({ ok: true });
    const returned = await pending;
    const stored = (await this.getMessages()).at(-1);
    return {
      returnedStatus: returned.status,
      storedStatus: (stored?.metadata as { status?: unknown } | undefined)
        ?.status,
      hookStatus: this._responseLog.at(-1)?.status
    };
  }
  ...
  /** Answer the next structured turn's forced final-answer tool with `args`. */
  ...
  override getModel(): LanguageModel {
        ...
        doStream: async () => ({
          stream: new ReadableStream({
            start: async (controller) => {
              controller.enqueue({ type: "stream-start", warnings: [] });
              controller.enqueue({ type: "text-start", id: "mid" });
              controller.enqueue({
                type: "text-delta",
                id: "mid",
                delta: "first"
              });
              this._midStreamEntered = true;
              await midStream;
              controller.enqueue({
                type: "text-delta",
                id: "mid",
              ...
              controller.enqueue({ type: "text-end", id: "mid" });
              controller.enqueue({
                type: "finish",
                finishReason: v3FinishReason("stop"),
                usage: v3Usage(10, 5)
    ...
    const overlap = this._tailOverlap;
    if (overlap) {
      return new MockLanguageModelV3({
        doStream: async () => ({
          stream: new ReadableStream({
            start: async (controller) => {
              controller.enqueue({ type: "stream-start", warnings: [] });
              controller.enqueue({ type: "text-start", id: "overlap" });
              await overlap.whenAdvanced;
              ...
              await this.reportProgress({ milestone: "overlap-ready" });
              controller.enqueue({
                type: "text-delta",
                id: "overlap",
                delta: "overlap-once"
              });
              await overlap.whenFinished;
              controller.enqueue({ type: "text-end", id: "overlap" });
              controller.enqueue({
                type: "finish",
                finishReason: v3FinishReason("stop"),
                usage: v3Usage(10, 5)
...
export class ThinkAgentToolParent extends Agent {
  ...
  private _heldFacetLeases: Array<{
    childName: string;
    token: string | null;
    disposed: boolean;
    wait: Promise<void>;
  ...
  async prepareApprovalFacetForTest(name: string): Promise<void> {
    const child = await this.subAgent(ThinkToolsTestAgent, name);
    await child.useEchoActionForTest("approval");
  }

  async approvalFacetProbeForTest(name: string): Promise<{
    preparations: number;
    responses: number;
    actionExecutions: number;
  }> {
    const child = await this.subAgent(ThinkToolsTestAgent, name);
    const result = await child.continuationAdmissionProbeForTest();
    try {
      // Return a DTO, not the native child RPC result's disposal capability.
      return {
      ...
      (result as typeof result & Disposable)[Symbol.dispose]();
    }
  }

  async stopApprovalFacetForTest(
    name: string,
    operation: "stop" | "cancel-child"
  ): Promise<void> {
    const child = await this.subAgent(ThinkToolsTestAgent, name);
    if (operation === "stop") await child.stopCurrentWork();
    else await child.cancelAgentToolRun(name, "Stopped by test");
  }

  holdNextFacetLeaseForTest(childName: string): number {
    let release!: () => void;
    const wait = new Promise<void>((resolve) => {
      release = resolve;
    });
    return (
      this._heldFacetLeases.push({
        childName,
        token: null,
        disposed: false,
        wait,
  ...
  facetLeaseProbeForTest(index: number): {
    acquired: boolean;
    disposed: boolean;
  } {
    const lease = this._heldFacetLeases[index];
    return { acquired: lease.token !== null, disposed: lease.disposed };
  }

  releaseFacetLeaseForTest(index: number): void {
    this._heldFacetLeases[index].release();
  }

  override async _cf_acquireFacetKeepAlive(
    ownerPath: ReadonlyArray<AgentPathStep>
  ): Promise<string> {
    const token = await super._cf_acquireFacetKeepAlive(ownerPath);
    const lease = this._heldFacetLeases.find(
      (candidate) =>
        candidate.token === null &&
        candidate.childName === ownerPath[ownerPath.length - 1]?.name
    );
    if (lease) {
      // Hold only the RPC reply: the real parent has already acquired this
      // native facet lease, and the child must dispose that exact token.
      lease.token = token;
      await lease.wait;
    }
    return token;
  ...
  override async _cf_releaseFacetKeepAlive(token: string): Promise<void> {
    await super._cf_releaseFacetKeepAlive(token);
    const lease = this._heldFacetLeases.find(
      (candidate) => candidate.token === token
    );
    if (lease) lease.disposed = true;
  }
  ...
  async reattachStuckTailableThinkChildForTest(
    ...
    const child = await this.subAgent(ThinkTestAgent, runId);
    // A provider that stays silent until aborted keeps the child running for
    // the re-attach budget, but lets cleanup await real cancellation settlement.
    await child.holdAgentToolModelForTest();
  ...
  async reattachMaxWindowExhaustedThinkChildForTest(
    ...
    const child = await this.subAgent(ThinkTestAgent, runId);
    // The provider stays silent until the ceiling cancels its AbortSignal.
    await child.holdAgentToolModelForTest();
  ...
  async reconcileParallelThinkChildrenForTest(): Promise<{
    ...
    await stuckChild.holdAgentToolModelForTest();
...
export class ThinkToolsTestAgent extends Think {
  override maxSteps = 3;
  private _preparationCount = 0;

  continuationAdmissionProbeForTest(): {
    preparations: number;
    responses: number;
    actionExecutions: number;
  } {
    return {
      preparations: this._preparationCount,
      responses: this._responseLog.length,
      actionExecutions: this._actionExecutionCount
    };
  }

  async stopDuringChatStartForTest(): Promise<{
    preparations: number;
    done: boolean;
    error?: string;
  }> {
    const collector = new TestCollectingCallback();
    const preparations = this._preparationCount;
    await this.chat("Stop before callback admission", {
      onStart: async (event) => {
        collector.onStart(event);
        await this.stopCurrentWork();
      },
      onEvent: (json) => collector.onEvent(json),
      onDone: () => collector.onDone(),
      onError: (error) => collector.onError(error)
    });
    return {
      preparations: this._preparationCount - preparations,
      done: collector.doneCalled,
      ...(collector.errorMessage ? { error: collector.errorMessage } : {})
    };
  }
  ...
  override async getActions(): Promise<Record<string, Action>> {
    const probe = this._durablePauseStopProbe;
    if (probe?.phase === "lookup") {
      probe.entered();
      try {
        await probe.wait;
      } finally {
  ...
  private _durablePauseStopProbe?: {
    phase: "lookup" | "execute";
    entered: () => void;
    wait: Promise<void>;
    aborted: boolean;
    finished: boolean;
  ...
  /** Stop across a real asynchronous action lookup or an executing action. */
  async stopDurablePauseForTest(
    executionId: string,
    phase: "lookup" | "execute",
    childRunId?: string
  ): Promise<{
    ...
    acknowledgedWhileHeld: boolean;
    finishedWhenAcknowledged: boolean;
    beforeSteps: number;
    afterSteps: number;
    stable: boolean;
  }> {
    ...
    const probe = { phase, entered, wait, aborted: false, finished: false };
    this._durablePauseStopProbe = probe;
    const beforeSteps = this._beforeStepLog.length;
    const approval = this.approveExecution(executionId);
    try {
      await Promise.race([
        enteredPromise,
        approval.then(() => {
          throw new Error("Approval settled before the held action boundary");
        })
      ]);
      let acknowledged = false;
      let finishedWhenAcknowledged = false;
      const stopping = (
        childRunId
          ? this.cancelAgentToolRun(childRunId, "Stopped by test")
          : this.stopCurrentWork()
      ).then(() => {
        acknowledged = true;
        finishedWhenAcknowledged = probe.finished;
      });
      // Yield one event-loop turn while the public hook/action remains held;
      // an early Stop acknowledgement cannot hide behind promise scheduling.
      await new Promise<void>((resolve) => setTimeout(resolve, 0));
      const acknowledgedWhileHeld = acknowledged;
      const signalAborted = probe.aborted;
      release();
      const [output] = await Promise.all([approval, stopping]);
      const stable = await this.waitUntilStable({ timeout: 5000 });
      return {
        outputJson: JSON.stringify(output),
        signalAborted,
        acknowledgedWhileHeld,
        finishedWhenAcknowledged,
        beforeSteps,
        afterSteps: this._beforeStepLog.length,
        stable
      };
    } finally {
      release();
      this._durablePauseStopProbe = undefined;
    }
  }
  ...
  /** Derive a descriptor for a paused action output for unit tests. */
  ...
  async getChildRunStatusForTest(runId: string) {
    return (await this.inspectAgentToolRun(runId))?.status ?? null;
  }
...
export class ThinkRecoveryTestAgent extends Think {
  ...
  private _stopTestActions: Record<string, Action> = {};
  private _stopTestModel: LanguageModel | undefined;
  private _stopTestBeforeTool:
    | ((ctx: ToolCallContext) => Promise<void>)
    | undefined;

  override getTools(): ToolSet {
    return { ...super.getTools(), ...this._stopTestTools };
  }

  override getActions(): Record<string, Action> {
    return this._stopTestActions;
  }

  override beforeToolCall(ctx: ToolCallContext): Promise<void> | void {
    return this._stopTestBeforeTool?.(ctx);
  }

  async stopWithHeldToolForTest(
    phase: "decision" | "cleanup",
    kind: "scalar" | "streaming" | "action" = "scalar"
  ) {
    ...
    const markerEntered = new Promise<void>((resolve) => {
      enteredMarker = resolve;
    });
    let toolCalls = 0;
    let toolFinished = false;
    let finishedWhenStopAcknowledged = false;
    this._stopTestBeforeTool = async (ctx) => {
      if (ctx.toolName === "stop_tool" && phase === "decision") {
        enteredTool();
        await heldTool;
      }
    };
    ...
    this._stopTestTools = {
      abort_marker: tool({
        ...
        execute: async (_input, { abortSignal }) => {
          if (!abortSignal)
            throw new Error("Expected the real tool abort signal");
          enteredMarker();
          ...
          return "parallel tool observed cancellation";
        }
      })
    };
    if (kind === "action") {
      this._stopTestActions.stop_tool = action({
        description: "Hold the tool until cancellation cleanup is released",
        inputSchema: z.object({}),
        execute
      });
    } else {
      this._stopTestTools.stop_tool = tool({
        inputSchema: z.object({}),
        execute:
          kind === "streaming"
            ? async function* () {
    ...
    this._stopTestModel = new MockLanguageModelV3({
      doStream: async () => ({
        stream: new ReadableStream({
          start: (controller) => {
            controller.enqueue({ type: "stream-start", warnings: [] });
            for (const name of ["stop_tool", "abort_marker"]) {
              controller.enqueue({
                type: "tool-input-start",
                id: name,
                toolName: name
              ...
              controller.enqueue({ type: "tool-input-end", id: name });
              controller.enqueue({
                type: "tool-call",
                toolCallId: name,
                toolName: name,
    ...
    const turn = this.testChat("Run the tool.");
    await Promise.race([
      Promise.all([entered, markerEntered]),
      turn.then((result) => {
        throw new Error(
          `Turn finished before tool entry: ${JSON.stringify(result)}`
        );
      })
    ]);
    const stopping = this.stopCurrentWork().then(() => {
      finishedWhenStopAcknowledged = toolFinished;
    });
    try {
      if (phase === "cleanup") {
        // Native stream/queue completion is intentionally weaker than the
        // tool's own settlement; Stop must still wait for the held operation.
        await turn;
      ...
      this._stopTestModel = undefined;
      this._stopTestTools = {};
      this._stopTestActions = {};
      this._stopTestBeforeTool = undefined;
    }
    return { toolCalls, finishedWhenStopAcknowledged };
  }

  async holdRecoveryModelForTest(): Promise<void> {
    this._holdModel = true;
  }

  async getHeldRecoveryModelForTest() {
    return { calls: this._heldModelCalls, aborted: this._heldModelAborted };
  }

  async releaseRecoveryModelForTest(): Promise<void> {
    this._holdModel = false;
  ...
  async getStoredTextForTest(): Promise<string[]> {
    return (await this.session.getHistory()).flatMap((message) =>
      message.parts.flatMap((part) =>
        part.type === "text" ? [z.string().parse(part.text)] : []
      )
    );
  }

  async startAgentToolRunStatusForTest(input: string, runId: string) {
    return (await this.startAgentToolRun(input, { runId })).status;
  }
  ...
  async getSubmissionMessagesForTest(
    submissionId: string
  ): Promise<string | null> {
    return (
      this.sql<{ messages_json: string }>`
      SELECT messages_json FROM cf_think_submissions WHERE submission_id = ${submissionId}
          ...
          return {
            stream: new ReadableStream({
              start: (controller) => {
                ...
                const abort = () => {
                  this._heldModelAborted = true;
                  this._releaseHeldModel = undefined;
                  controller.error(abortSignal?.reason ?? new Error("Aborted"));
                };
                this._releaseHeldModel = () => {
                  abortSignal?.removeEventListener("abort", abort);
                  this._releaseHeldModel = undefined;
                  controller.enqueue({ type: "text-end", id: "held" });
                  controller.enqueue({
                    type: "finish",
                    finishReason: v3FinishReason("stop"),
                    usage: v3Usage(10, 5)
  ...
  /** Stop while native admission is awaiting the existing terminal-status read. */
  async stopDuringChatAdmissionForTest(): Promise<string[]> {
    const frames: string[] = [];
    const connection = {
      id: "c-stop-admission",
      send(frame: string) {
    ...
    const admission = self._handleChatRequest(connection, {
      type: "chat-request",
          ...
          messages: [
            {
              id: "stopped-input",
              role: "user",
              parts: [{ type: "text", text: "Do not start after Stop" }]
            }
          ]
        })
      }
  ...
  /**
   * Vendor divergence: seed a chat turn interrupted mid-run on the TASKS
   * capability — the post-eviction shape of a root agent's turn at 0.23,
   * where `insertInterruptedFiber`'s `cf_agents_runs` row only covers
   * facet-hosted turns. Enqueued (never attached), so no live closure is
   * registered and the first drive takes the replay path a fresh isolate
   * takes: `handleRecovery` into the shared ChatRecoveryEngine.
   */
  async insertInterruptedChatTurnTaskForTest(
    requestId: string,
    snapshot?: unknown
  ): Promise<string> {
    ...
    if (snapshot !== undefined) {
      await this.ctx.storage.put(`__cf_chat_turn_snapshot:${runId}`, snapshot);
    }
    await this.tasks.__DO_NOT_USE_WILL_BREAK__enqueue(
      (this.constructor as typeof Think).CHAT_FIBER_NAME,
      { requestId, continuation: false, nonce },
      { runId, retain: false, metadata: { requestId } }
    );
    // Park it in the future so the wake this run is due for cannot fire
    // before the test has recorded its Stop; `runDueTasksForTest` pulls it
    // back into the past.
    const later = Date.now() + 60 * 60 * 1000;
    this.sql`
      UPDATE cf_agents_task_runs SET next_at = ${later} WHERE run_id = ${runId}
    `;
    this
      .sql`UPDATE cf_agents_jobs SET time = ${later} WHERE id = ${`task:${runId}`}`;
    return runId;
  }

  /** Drive every due Task run and Lifecycle job, as an alarm wake does. */
  async runDueTasksForTest(): Promise<void> {
    const past = Date.now() - 1_000;
    this.sql`
      UPDATE cf_agents_task_runs SET next_at = ${past}
      WHERE state IN ('pending', 'running', 'waiting')
    `;
    this.sql`UPDATE cf_agents_jobs SET time = ${past}`;
    await this.alarm();
    await (
      this as unknown as { _turnQueue: { waitForIdle(): Promise<void> } }
    )._turnQueue.waitForIdle();
  }
