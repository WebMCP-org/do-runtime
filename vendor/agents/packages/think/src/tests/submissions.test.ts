...
describe("Think durable submissions", () => {
  ...
  it("stamps submission metadata on the turn's user message", async () => {
    const agent = await freshAgent();

    await agent.testSubmitMessages("queued work", {
      submissionId: "sub-turn-metadata",
      metadata: { source: "agents-as-tools-background", runId: "notify-run" }
    });
    await waitForSubmission(
      agent,
      "sub-turn-metadata",
    ...
    // The same `metadata.turnMetadata` stamp `chat()` writes: the notice a
    // detached run submits is only distinguishable from a human message on the
    // durable record, and `activeTurnMetadata` re-resolves it from there.
    const user = (await agent.getMessagesForTest()).find(
  ...
  it("keeps only the completed stream, not the outcome stamp, after a submission settles", async () => {
    const agent = await freshAgent();
    ...
    // Vendor divergence (adapted): upstream discards the stream rows at
    // cutover; the fork retains the completed record until the next stream
    // starts (2026-09-14 "Preserve completion across a cold restart"). Ledger
    // settlement still clears the outcome stamp, and a resume answers idle.
    await expect(
