...
describe("Think — core", () => {
  ...
  it("exposes persisted early agent-tool chunks while the child is running", async () => {
    const agent = await freshAgent(
      `agent-tool-running-backlog-${crypto.randomUUID()}`
    );
    const backlog = await agent.readRunningAgentToolBacklogForTest(
      crypto.randomUUID()
    );

    expect(backlog.status).toBe("running");
    expect(backlog.storedChunks.length).toBeGreaterThan(0);
    expect(
      backlog.visibleChunks.map(({ sequence, body }) => ({ sequence, body }))
    ).toEqual(backlog.storedChunks);
    for (const chunk of backlog.visibleChunks) {
      expect(chunk.cursor).toEqual({
        epoch: backlog.streamId,
  ...
  it("should use custom response from setResponse", async () => {
    ...
    const messages = await agent.getStoredMessages();
    expect(messages).toHaveLength(2);
    const userMsg = messages[0] as UIMessage<ThinkMessageMetadata>;
    const assistantMsg = messages[1] as UIMessage<ThinkMessageMetadata>;
    expect(assistantMsg.role).toBe("assistant");
    const textParts = assistantMsg.parts.filter((p) => p.type === "text");
    const fullText = textParts.map((p) => p.text ?? "").join("");
    expect(fullText).toBe("Custom response text");
    expect(typeof assistantMsg.metadata?.createdAt).toBe("string");
    expect(assistantMsg.metadata?.createdAt).toBe(userMsg.metadata?.createdAt);
    expect(assistantMsg.metadata?.status).toBe("completed");
    expect(assistantMsg.metadata?.durationMs).toBeGreaterThanOrEqual(0);
  });
...
describe("Think — error handling", () => {
  ...
  it("should persist partial assistant message on error", async () => {
    ...
    const assistantMsg = history[1] as UIMessage<ThinkMessageMetadata>;
    expect(assistantMsg.role).toBe("assistant");
    expect(assistantMsg.parts.length).toBeGreaterThan(0);
    expect(assistantMsg.metadata?.status).toBe("error");
    expect(assistantMsg.metadata?.durationMs).toBeGreaterThanOrEqual(0);
  });
...
describe("Think — abort", () => {
  ...
  it("should persist partial message on abort", async () => {
    ...
    expect(assistantMsg.role).toBe("assistant");
    expect(assistantMsg.parts.length).toBeGreaterThan(0);
    expect(assistantMsg.metadata?.status).toBe("aborted");
    expect(assistantMsg.metadata?.durationMs).toBeGreaterThanOrEqual(0);
  });

  it("keeps abort status while structured output settles", async () => {
    const agent = await freshAgent("abort-structured-output-settle");

    const result = await agent.abortWhileStructuredOutputSettlesForTest();

    expect(result).toEqual({
      returnedStatus: "aborted",
      storedStatus: "aborted",
      hookStatus: "aborted"
    });
  });
...
describe("Think — Session integration", () => {
  ...
  it("exposes live and restored creation times as message metadata", async () => {
    const agent = await freshAgent("session-message-time");
    await agent.appendHistoryMessageForTest({
      id: "stamped-user",
      role: "user",
      parts: [{ type: "text", text: "Stamped by Think" }]
    });

    const live =
      (await agent.getStoredMessages()) as UIMessage<ThinkMessageMetadata>[];
    expect(typeof live[0].metadata?.createdAt).toBe("string");

    await agent.appendSessionMessageForTest({
      id: "session-user",
      role: "user",
      parts: [{ type: "text", text: "Stamped by Session storage" }]
    });
    const sessionHistory = (await agent.getSessionHistoryForTest()) as Array<
      UIMessage & { createdAt?: Date }
    >;
    const sessionCreatedAt = sessionHistory[1].createdAt;
    const restored =
      (await agent.syncMessagesFromStorageForTest()) as UIMessage<ThinkMessageMetadata>[];

    expect(sessionCreatedAt).toBeInstanceOf(Date);
    expect(restored[1].metadata?.createdAt).toBe(
      sessionCreatedAt?.toISOString()
    );
  });

  it("replaces invalid trusted creation metadata at append", async () => {
    const agent = await freshAgent("session-message-time-invalid");
    await agent.appendHistoryMessageForTest({
      id: "invalid-time",
      role: "user",
      parts: [{ type: "text", text: "Invalid time" }],
    ...
    expect(stored.metadata?.createdAt).not.toBe("1970-01-01T00:00:00.000Z");
  });

  it("owns creation metadata across browser intake and replay", async () => {
    const agent = await freshAgent("session-message-time-intake");
    await agent.testChat("Seed a terminal assistant message");
    const before = (
      await agent.getStoredMessages()
    )[1] as UIMessage<ThinkMessageMetadata>;
    const forgedReplay: UIMessage & { createdAt: string } = {
      ...before,
      ...
      metadata: {
        ...
        durationMs: 1,
        status: "error",
        applicationFact: "preserved"
      }
    };
    await agent.persistIncomingMessageForTest(forgedReplay);

    const replayed = (await agent.getStoredMessages())[1] as UIMessage<
      ThinkMessageMetadata & { applicationFact?: string }
    >;
    expect(replayed.metadata?.createdAt).toBe(before.metadata?.createdAt);
    expect(replayed.metadata?.durationMs).toBe(before.metadata?.durationMs);
    expect(replayed.metadata?.status).toBe(before.metadata?.status);
    expect(replayed.metadata?.applicationFact).toBe("preserved");

    const forgedUser: UIMessage & { createdAt: string } = {
      id: "forged-user",
      role: "user",
      parts: [{ type: "text", text: "Hello" }],
      createdAt: "2000-01-01T00:00:00.000Z",
    ...
    await agent.persistIncomingMessageForTest(forgedUser);

    const messages = (await agent.getStoredMessages()) as UIMessage<
      ThinkMessageMetadata & { applicationFact?: string }
    >[];
    const message = messages.find(({ id }) => id === forgedUser.id)!;
    const metadata = message.metadata as Record<string, unknown>;
    expect(metadata.createdAt).not.toBe("2000-01-01T00:00:00.000Z");
    expect(metadata.durationMs).toBeUndefined();
    expect(metadata.status).toBeUndefined();
    expect(metadata.applicationFact).toBe("preserved");
  });
...
describe("Think — onChatResponse", () => {
  ...
  it("does not rewrite a completed message when terminal delivery fails", async () => {
    const room = `hook-done-callback-error-${crypto.randomUUID()}`;
    const finishStatuses: unknown[] = [];
    const unsubscribe = subscribe("chat", (event) => {
      if (event.name === room && event.type === "chat:turn:finish") {
        finishStatuses.push(event.payload.status);
      }
    });
    const agent = await freshAgent(room);

    try {
      await agent.testChatWithThrowingDoneCallback("Hello!");

      const history =
        (await agent.getStoredMessages()) as UIMessage<ThinkMessageMetadata>[];
      expect(history[1].metadata?.status).toBe("completed");
      expect(history[1].metadata?.durationMs).toBeGreaterThanOrEqual(0);
      const log = (await agent.getResponseLog()) as ChatResponseResult[];
      expect(log.at(-1)?.status).toBe("completed");
      expect(finishStatuses).toEqual(["completed"]);
    } finally {
      unsubscribe();
    }
  });
...
describe("Think — saveMessages with external AbortSignal", () => {
  ...
  it("continueLastTurn pre-aborted skips preparation and reports aborted", async () => {
    const agent = await freshProgrammaticAgent("ext-abort-continue-pre");
    await agent.testChat("seed");
    const preparedTurns = await agent.getCapturedOptions();
    const history = await agent.getStoredMessages();
    ...
    expect(result.status).toBe("aborted");
    expect(await agent.getCapturedOptions()).toEqual(preparedTurns);
    expect(await agent.getStoredMessages()).toEqual(history);
  });
});
...
describe("Think — onChatRecovery", () => {
  ...
  it("drops a provider-replayed tool call from a continuation", async () => {
    const agent = await freshAgent(`provider-replay-${crypto.randomUUID()}`);

    const parts = await agent.runProviderReplayContinuationForTest();

    expect(
      parts.filter((part) => part.toolCallId === "replayed-tool")
    ).toHaveLength(1);
    expect(
      parts.some((part) => part.type === "text" && part.text === "continued")
    ).toBe(true);
  });
  ...
  it("persists the settled partial when the recovery budget is exhausted (#1631)", async () => {
    ...
    expect(messages).toHaveLength(1);
    expect(messages[0].role).toBe("assistant");
    const timing = messages[0].metadata as ThinkMessageMetadata | undefined;
    expect(typeof timing?.createdAt).toBe("string");
  ...
  it("never drops settled tool results on { persist: false } — preserves them anyway (#1631)", async () => {
    ...
    // assistant partial carrying the completed tool call IS persisted.
    expect(await agent.getRecoveryContexts()).toHaveLength(1);
    const messages = (await agent.getStoredMessages()) as UIMessage[];
    expect(messages).toHaveLength(1);
    expect(messages[0].role).toBe("assistant");
  ...
  it("honors { persist: false } for a text-only partial with no settled work (#1631)", async () => {
    ...
    await agent.insertInterruptedFiber("__cf_internal_chat_turn:req-textonly");

    await agent.triggerFiberRecovery();

    // No settled tool results to preserve, so `persist: false` is honored —
    // the recovery hook runs and nothing is persisted.
    expect(await agent.getRecoveryContexts()).toHaveLength(1);
    const messages = (await agent.getStoredMessages()) as UIMessage[];
    expect(messages).toHaveLength(0);
  });
