...
Audited 2026-09-11 (Pacific). This measures source `5cae3006be02`, whose tree is
identical to merged `ff87e9ef`, against the released upstream baseline in
...
minimal: the audit found a missed sibling caller, an inherited React risk and
an untested continuation path. Those are better next steps than adding a host
workaround or deleting a fence because its name resembles another one.

| Priority                           | Evidence                                                                                                                                                                                                                                                                                                  | Smallest next action                                                                                                                                                                                                         |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Incomplete method-identity fix     | Think skill initialization uses `_isAgentMethodOverride`, but `_isRecoverableContextOverflow` still compares the wrapped `classifyChatError` directly with its prototype. [#2165's discussion](https://github.com/cloudflare/agents/issues/2165#issuecomment-5629567318) identifies the same sibling bug. | Reuse the existing helper at the second comparison and add one reactive-overflow regression with no classifier override. Do not combine the contributor's alternative wrapper strategy with ours.                            |
| Inherited React cleanup dispatch   | The exact unconditional `setClientToolResults` effect in open [#2217](https://github.com/cloudflare/agents/issues/2217) remains in both our pin and upstream main. The reporter provides a measured reproduction and a proposed pre-dispatch staleness check.                                             | Reproduce with the actual long-stream hook before adapting the fix. Raising throttle would mask the dispatch source. This audit did not reproduce a new Rook failure.                                                        |
| Continuation reattachment coverage | Observer replay uses an immutable seed, but transport-owned continuation replay skips the hydrated reset. Our new hydrated-assistant test covers ordinary replay. Open [#1951](https://github.com/cloudflare/agents/issues/1951) describes a continuation suffix arriving twice.                          | Add a real-hook continuation/route-replacement reproduction. Do not claim observer-seed and ordinary-hydration regressions prove this case.                                                                                  |
...
The first finding is source-confirmed; the next two need the specified runnable
reproductions. No runtime behavior, dependency pin or storage format changes as
part of this audit.
...
| Agents    |                   25 |       2,535 |           569 |
| AI Chat   |                    1 |          51 |            26 |
| Think     |                   15 |       2,499 |         1,132 |
| Voice     |                    3 |          75 |            27 |
| Shell     |                    9 |       1,917 |            80 |
| Codemode  |                    0 |           0 |             0 |
| **Total** |               **53** |   **7,077** |     **1,834** |
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| [#1951 continuation replay](https://github.com/cloudflare/agents/issues/1951)                                                                                                                                                                                                             | Open; contributor recommends immutable pre-continuation seed or segment identity.                                                                                                  | Supports the existing observer seed. Transport-owned reattachment still needs proof.                                                                 |
...
| [#2217 React cleanup](https://github.com/cloudflare/agents/issues/2217), [contributor change](https://github.com/Konan69/agents/commit/ce283f35)                                                                                                                                          | Open; reporter's fix checks for stale entries before dispatch, then rechecks current state in the updater.                                                                         | Useful exact lead, not an adopted or locally verified fix. Use sequencing assertions rather than copying a brittle paint deadline.                   |
...
| [#2165 method identity](https://github.com/cloudflare/agents/issues/2165)                                                                                                                                                                                                                 | Open; discussion identifies both the skills warning and reactive-overflow sibling comparison.                                                                                      | Complete the existing helper's caller coverage; no second wrapper layer.                                                                             |
...
Those runs establish the existing tested baseline, not coverage for the new
questions identified above. Documentation edits require formatting and link
checks; runtime follow-ups require their new reproductions and owning gates.
