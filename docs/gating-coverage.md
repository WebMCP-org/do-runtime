...
| Actor-created `TransformStream` callbacks | constructor captures the current actor and async stores; `transform`, `flush`, and `cancel` use `makeReentryCallback`; synchronous `start` retains its native timing and receiver | `api/global-scope.ts`; delayed input regression in `global-scope.test.ts` |
